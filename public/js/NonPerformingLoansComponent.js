...
var NonPerformingLoansComponent = new function(){
    this.base_url = $('#__base_url').val();
    let mThis = this;
    this.self = $('#_mainNonPerformingLoansComponent');

    this.tblNonPerformingLoans = $('#_mainNonPerformingLoan_table');

    this.btnAddNonPerformingLoan = $('#btnAddNonPerformingLoan');

    this.init = () => {
        mThis.btnAddNonPerformingLoan.on('click', () => { 
            let op = {title:'New Non-Performing Loan'};
            NonPerformingLoanDialog.show(op, (p) => {
                //alert(JSON.stringify(p));
                post_ajax(mThis.base_url + '/api/save-applicant', p, function(results){
                    if(results.status == 'OK'){
                        mThis.displayNonPerformingLoanList();
                    }else{
    ...
    this.displayNonPerformingLoanList = function()
    { 
        var p = null;
        post_ajax([mThis.base_url, '/api/applicant-list'].join(''),p,function(data) {  
            ...
            if (mThis.table){
                    mThis.tblNonPerformingLoans.DataTable().clear().destroy();
                    //NOTE that ...DataTable().clear() will clear only tbody, and NOT <thead> section, so we need to ensure that the target table is cleared all, remmining only tags "<table></table>"
                    mThis.tblNonPerformingLoans.empty();
                    //alert('destroyed => '+  mThis.tblNonPerformingLoans.html());
                    mThis.table = null;
            }
            ...
            if (!mThis.table)
            mThis.table = mThis.tblNonPerformingLoans.DataTable({
                searching:false,
                destroy:true,
                paging:true,
                ordering:false,
            ...
            // $('#_dl_tblNonPerformingLoans_wrapper>div.dt-buttons').prepend(div);
...
var NonPerformingLoanDialog = new function(){
    let mThis = this;
    this.self = $('#modalAddNonPerformingLoan');
    this.btnSave = $('#modalAddNonPerformingLoan_btnSave');
    this.elTitle = $('#modalAddNonPerformingLoan_title');

    this.elErrors = $('#modalAddNonPerformingLoan_error');
...
$(document).ready(function(){
    NonPerformingLoansComponent.init();
});
...
var PDFReceipt = new function(){
    ...
    this.getReceiptContent = (d)=>{
        ...
        do{
            ...
            c = d.items[i];
            let amount =0;
            if ((c.description).toLowerCase() =='discount' || (c.description).toLowerCase() =='discount amount')
              amount = ['(',d.cur,Number(c.amount).toFixed(2),')'].join('');
            else amount = [d.cur,c.amount].join('');
       ...
       content= [
            ...
            {text:`NO: ${d.receipt_number}`,fontSize:12,bold:true,alignment:'center'},
            {
                table: {
                    ...
                    body: [
                    [
                        {text:'Student Name',fontSize:10,alignment:'right'},
                        {text:`: ${d.payer_name}`,alignment:'left'},
                        {text:'Receipt No',fontSize:10,alignment:'right'},
                        {text:`: ${d.receipt_number}`,fontSize:10,alignment:'left'}
                    ],
                   [
                    {text:'Student ID',fontSize:10,marginTop:-5,alignment:'right'},
                    {text:`: ${d.payer_code}`,marginTop:-5,alignment:'left'},
                    {text:'Payment date',fontSize:10,marginTop:-5,alignment:'right'},
                    {text:`: ${d.payment_date}`,fontSize:10,marginTop:-5,alignment:'left'} 
                   ],
                   [
                    {text:'Phone number',fontSize:10,marginTop:-5,alignment:'right'},
                    {text:`: ${d.phone_number}`,marginTop:-5,alignment:'left'},
                    {text:'Issue date',fontSize:10,marginTop:-5,alignment:'right'},
                    {text:`: ${d.issue_date}`,fontSize:10,marginTop:-5,alignment:'left'} 
                   ]
                ]},
                layout:'noBorders'
                // layout: {
            ...
            [
                   ...
                   {text:`Total Amount: ${[d.cur,d.total].join('')}`,alignment:'right',marginRight:15,bold:true,fontSize:12}
             ],
            {text:'Receiver`s signature:......................................',alignment:'right',fontSize:11,marginRight:15,marginTop:5,bold:true},
            {text:`Name: ${d.create_user}`,alignment:'left',fontSize:11,marginLeft:310,bold:true}
    ...
    //d = {payment_date,receipt_number, payer_name, amount, principal_amount, interest_amount,client_remarks}
    ...
    this.createContent =(d,option)=> {
          ...
          if (option.copies_per_page==2)    
             receipt_contents=[mThis.getReceiptContent(d),{'text':' ',marginTop:35},mThis.getReceiptContent(d)]; 
  ...
  this.show = (data,option)=>{ 
      let docDef = mThis.createContent(data,option); 
   ...
   this.download = (data,option)=>{ 
    let docDef = mThis.createContent(data,option); 
    /**NOTE:  _vfs_fonts is "Virtual File System fonts" defined in javascript file "vfs_fonts.js" that is in the same directory with file pdfMake.min.js **/
    pdfMake.vfs = _vfs_fonts; // _vfs_fonts is built using node command. 'node build-vfs.js "./examples/fonts" '
    //if(option.styles) pdfMake.styles = option.styles;
    if (docDef) pdfMake.createPdf(docDef,null,null).download(option.file_name);
    else cv_interact.alert('It seems no data to display. If you see data, make sure your searchbox is empty');
 }

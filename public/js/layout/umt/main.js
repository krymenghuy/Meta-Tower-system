...
let main_view = new function(){
    ...
    this.getEncryptData = (qstring,onFinish)=>{
        let p = {'data':qstring};
        vsapi.call1([mThis.base_url,'/dms/encryptData'].join(''),p).then((res)=>{
    ...
    this.displayNotifications = ()=> {
        vsapi.call1(`${mThis.base_url}/dms/notifications`,null).then((res)=>{
            let i=0;
    ...
    this.displayTasks =()=> {
        let items =null;
        vsapi.call1(`${mThis.base_url}/dms/pending-requests`,null).then((res)=>{
            if(res.status_code===200){
                let d = res.data;
                let i=0;
                let c;
    ...
    mThis.updateNotificationCount = ()=>{
        vsapi.call1(`${mThis.base_url}/dms/unread-count`,null,false).then((res)=>{
            if(res.status_code ===200){
                let d = res.data;

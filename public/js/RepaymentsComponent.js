...
var RepaymentsComponent = new function(){
    this.base_url = $('#__base_url').val();
    let mThis = this;
    this.self = $('#_mainRepaymentsComponent');

    this.tblRepayment = $('#_mainRepaymentsComponent_table');

    this.btnAddRepayment = $('#btnAddRepayment');

    this.init = () => {
        mThis.btnAddRepayment.on('click', () => { 
            let op = {title:'New Repayment List'};
            RepaymentDialog.show(op, (p) => {
                //alert(JSON.stringify(p));
                post_ajax(mThis.base_url + '/api/save-applicant', p, function(results){
                    if(results.status == 'OK'){
                        mThis.displayRepaymentList();
                    }else{
    ...
    this.displayRepaymentList = function()
    { 
        var p = null;
        post_ajax([mThis.base_url, '/api/applicant-list'].join(''),p,function(data) {  
            ...
            if (mThis.table){
                    mThis.tblRepayment.DataTable().clear().destroy();
                    //NOTE that ...DataTable().clear() will clear only tbody, and NOT <thead> section, so we need to ensure that the target table is cleared all, remmining only tags "<table></table>"
                    mThis.tblRepayment.empty();
                    //alert('destroyed => '+  mThis.tblRepayment.html());
                    mThis.table = null;
            }
            ...
            if (!mThis.table)
            mThis.table = mThis.tblRepayment.DataTable({
                searching:false,
                destroy:true,
                paging:true,
                ordering:false,
            ...
            // $('#_dl_tblRepayment_wrapper>div.dt-buttons').prepend(div);
...
var RepaymentDialog = new function(){
    let mThis = this;
    this.self = $('#modalRepayment');
    this.btnSave = $('#modalRepayment_btnSave');
    this.elTitle = $('#modalRepayment_title');

    this.elErrors = $('#modalRepayment_error');

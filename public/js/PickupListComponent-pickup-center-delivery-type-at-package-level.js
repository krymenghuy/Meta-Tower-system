...
var PickupListComponent = new function() {
    ...
    this.pickup_dropdown_menu = mThis.tblPickups.find('div.dropdown');
    ...
    this.init = function() {
          ...
          //## begin::listen to Event Notifier
                ...
                let pusher = new Pusher(main_view.PUSHER_APP_KEY,{
                 ...
                 channel.bind('onNewPickupRequest', function(d){
                     let data = d.data;
                     ...
                     if(mThis.tblPickups.is(':visible')) {
                        ...
                        if (filter_status_id ==1) mThis.displayPickupList();
                     }
       ...
       //##BEGIN:: tblPickups dropdown menu
                ...
                $(document).on('click',function(e){
                    //e.preventDefault();
                    let container = mThis.pickup_dropdown_menu.parent(); // div.dropdown or $('#_pmt_stf_dropdown_container')
                    if(container){
                        if (!container.is(e.target) && container.has(e.target).length === 0) {
                            mThis.pickup_dropdown_menu.removeClass('show');
                        }
                    }
                });
    ...
    this.updatePickupStatus = (tr,status_info)=>{
        ...
        //alert(JSON.stringify(el.parent().html()));
    ...
    this.createDropdownMenuHtml_pickup = function(order_id, sender_id, status_id) {
        ...
        let html = ['<div class="dropdown-menu" data-orderid="',order_id,'" data-senderid="',sender_id,'" data-statusid="',status_id,'">',
          ...
          '<a class="dropdown-item _pl_pa_delete" href="#"><i class="fa fa-times" style="color:red"></i> Delete Pickup</a>',
                ...
                let my_columns = [
                    ...
                    },
                    {
                        ...
                        data:function(data,a,b){
                            ...
                            if(data.completed ==1) 
                               cls_completed = 'pl-order-completed';
                            else {
                                if (data.status_id ==2 || data.status_id ==1 || data.status_id ==0) 
                                   cls_completed ='pl-order-status-'+data.status_id;
                            }     
                            return ['<div><div class="',cls_completed,'"></div><span class="pl-order_code">',data.order_code,'</span></div>'].join('');
                        },
                        title:'Order ID'
                        // ,data:function(data,type,meta){
...
//begin::EditPickupDialog
var PickupDialog = new function() {
    let mThis = this;
    this.base_url = $('#__base_url').val();
    this.self = $('#_pl_dlgEditPickup');
    this.body = $('#_pl_dlgEditPickup_body');
    this.elTitle = $('#_pl_dlgEditPickupTitle');
    this.elVechicleType = $('#_pl_edit_vehicle_type');
    this.elCondition = $('#_pl_edit_request_condition');
    this.btnSave = $('#_pl_edit_btnSave');
    this.elError = $('#_pl_edit_error');
    this.lnkFindSender = $('#_pl_edit_find_sender');

    this.elSenderCode = $('#_pl_edit_sender_code');
    this.elSenderName = $('#_pl_edit_sender_name');

    this.elSenderCode.on('blur',function(e){
      mThis.setSenderName($(this).val());
    });
    
    this.elSenderCode.on('keyup',function(e){
       if (e.keyCode ==13) mThis.setSenderName($(this).val());
    });
    ...
    this.lnkFindSender.on('click',function(e){
        ...
        let option ={
            'title':'Find Merchant',
            'role':'sender', //find person of type "sender" or merchant
        ...
        FindPersonDialog.show(option,(ps)=>{
            if(ps[0]){
               mThis.elSenderCode.val(ps[0].code);
               mThis.elSenderName.val(ps[0].name);
            //    let p = {'sender_id':ps[0].id};
            //    post_ajax([mThis.base_url,'/api/getVendorAddress'].join(''),p,function(address){ 
            //        alert(address);
    ...
    this.btnSave.on('click',function(e){
        ...
        post_ajax([mThis.base_url,'/api/savePickupRequest'].join(''),p,function(res){
    ...
    this.setSenderName = (code) =>{
        let p = {'sender_code':code?code:''};
        post_ajax([mThis.base_url,'/api/getSenderInfoByCode'].join(''),p,function(d) {  
            if(typeof d =='string') return;
            if(d){
                d = StringSanitizer.sanitizeObject(d); 
                mThis.elSenderName.val(d.name).prop('readOnly',true);
            }
    ...
    this.prepareData= (def,onFinish)=>{
       ...
       if(mThis.form_options) {
          CommonLib.setComboItems(mThis.elVechicleType,mThis.form_options.vehicle_types,'vehicle_type','vehicle_type',true,'(Select Vehicle Type)',def.request_vehicle_type);
       ...
       post_ajax([mThis.base_url,'/api/getFormData_pickup_request'].join(''),null,function(data) {  
           ...
           CommonLib.setComboItems(mThis.elVechicleType,data.vehicle_types,'vehicle_type','vehicle_type',true,'(Select Vehicle Type)',def.request_vehicle_type);
    ...
    this.show = (option,onClose)=>{
        if(option) {
            if(option.title) mThis.elTitle.html(option.title);
            mThis.order_id = option.order_id;
        }
        mThis.onClose = onClose;
         
        if (mThis.order_id > 0) {
            mThis.elTitle.text('Pickup Details');
            let p = {'order_id':mThis.order_id};
            post_ajax([mThis.base_url,'/api/getPickupInfo'].join(''),p,function(d) { 
                d = StringSanitizer.sanitizeObject(d);
                if(d) {  
    ...
    this.setData = (d)=>{
     if(!d){
      ...
      mThis.body.find('.data-input').each(function() {
          let data_memebr = $(this).data('field');
          $(this).val(d[data_memebr]); 
      });
    }
    ...
    this.getData = (d)=>{
        ...
        mThis.body.find('.data-input').each(function() {
            let data_memebr = $(this).data('field');
            p[data_memebr] = $(this).val();
        });
        return p;
    }

}
//end::editPickupDialog
...
var PickupRequestDialog = new function() {
    ...
    //        data:function(data,type,row) {
    //            //option = {inputType, data, columnName,readOnly, valueMember,textMember,[defaultValue],'combo_items' }
    ...
    },
    {
      ...
      title:'Receiver Address' 
    },
       {
         ...
         data:function(data,a,b) {
           let d = {"inputType":"input","data":data,"columnName":"receiver_phone","cssClass":"phone","readOnly":false};
           return EditableTable.makeTableCellEditor(d);
         },
       ...
       },
       {
           ...
           data:function(data,a,b) {
               ...
               let d = {"inputType":"select","data":data,"columnName":"df_payer","readOnly":false,"combo_items":items,"valueMember":"name","textMamber":"name"};
               return EditableTable.makeTableCellEditor(d);
           },
           title:'DFP'
       },
       {
           ...
           data:function(data,a,b) {
               ...
               let d = {"inputType":"select","data":data,"columnName":"cod","readOnly":false,"combo_items":items,"valueMember":"id","textMamber":"name"};
               return EditableTable.makeTableCellEditor(d);
           },
           title:'COD'
       },
   ...
   //on PickupRequest Pickup Request Form
    ...
    this.show = (option,onClose) =>{
        ...
        mThis.elTitle.html(option.title?option.title:' New Pickup Request');
           mThis.prepareFormData(null,()=>{
            ...
            mThis.elPickupDate.val(DateHelper.getTodayDate());
    ...
    this.addPackageRow = ()=>{
        ...
        let row_data_blank = {'barcode':null,'delivery_type':null,'zone_name':null,'receiver_phone':null,'price':0,'base_fee':0,'delivery_fee':0,'df_payer':'Sendr','cod':0,'size':'','actual_kg':0,'billed_kg':0,'cod_fee':0}; 
    ...
    this.getData = function(){
      let p = {};
       ...
       p.receivers = mThis.getPackages(p.delivery_type);
...
//    }
   ...
   { 
       ...
       if (!data || !data[0]) { 
           ...
           data = [{'barcode':null,'delivery_type':null,'zone_name':null,'receiver_phone':null,'price':0,'base_fee':0,'delivery_fee':0,'df_payer':'Sendr','cod':0,'size':'','actual_kg':0,'billed_kg':0,'cod_fee':0}];
           //return;
       }
           if (mThis.table){
     ...
     this.performPickup = (pick_on_arrival=false, tr=null)=>{
            ...
            post_ajax([mThis.base_url,'/api/performPickup'].join(''),p,function(result){
                ...
                if(result) {
                    ...
                    if (result.success_count > 0) PickupListComponent.updatePickupStatus(tr, {'status_id':result.status_id,'status':result.status});
                }
            });
     }
...
//    }
    ...
    {   
        if (!data || !data[0]) { 
            //make one empty raw for data entry
            data = [{'barcode':null,'delivery_type':'normal','receiver_address':null,'receiver_phone':null,'receiver_name':null,'zone_code':null,'size':null,'actual_kg':0,'billed_kg':0,'price':0,'delivery_fee':0,'df_payer':'Sender','cod':0,'cod_fee':0,'forwarding_cost':'0'}];
            //return;
        }
            if (mThis.table){
...
var VerifyPackageDialog = new function() {
    ...
    // },
    {
        className:"zone_name", //zone_name is used as receiver_address
        width:'200px',
        data:function(data,a,b) {
            let d = {"inputType":"select","data":data,"columnName":"zone_name","cssClass":"modal-select2","combo_items": PickupListComponent.form_data.zones,"valueMember":"zone_code","textMember":"zone_name"};
            return EditableTable.makeTableCellEditor(d);
        },
        title:'Zone'
        },
...
// });
  ...
  this.setDeliveryPrices = (tr,change_agent) =>{
   ...
   { 
       if (!data || !data[0]) { 
           //Make one empty raw for data entry
           data = [{'barcode':null,'receiver_address':null,'receiver_phone':null,'receiver_name':null,'zone_code':null,'size':null,'actual_kg':0,'billed_kg':0,'price':0,'delivery_fee':0,'df_payer':'Sender','cod':0,'cod_fee':0,'forwarding_cost':'0'}];
           //return;
       }
  ...
  var FilterDialog_pickup = new function(){
     ...
     this.loadFilterData = (onFinish)=>{
        ...
        if (mThis.form_data) {
            ...
            CommonLib.setComboItems(mThis.elFilter_sender,mThis.form_data.senders,'id','sender_name',true,'(All Merchants)',def.sender_id);

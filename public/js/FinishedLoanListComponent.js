'use strict'
//begin::FinishedLoanListComponent
var FinishedLoanListComponent = new function(){
    let mThis = this;
    this.elScreenTitle = $('#screen_title');
    this.self = $('#_main_finishedLoanListComponent');
    this.base_url = $('#__base_url').val();
 
    this.tblLoanList = $('#_tblFinishedLoans');
  
    this.btnFind = $('#_finloan_btnSearch');
    this.elSearch = $('#_finloan_search');
 
    this.btnPrint = $('#_finloan_btnPrint');
    this.btnPDF = $('#_finloan_btnPDF');
    this.btnExcel = $('#_finloan_btnExcel');
  
     this.createDropdownMenuHtml_loan =(loan_id,status_id,person_id)=> {
         return null;
        //  //cla = 'class_list_action' = > cla_delete, cla_modify,...
        //  let html = ['<div class="dropdown-menu" data-id="',loan_id,'" data-statusid="',status_id,'">',
        //  '<a data-id="',loan_id,'" data-personid="',person_id,'" class="dropdown-item _finloan_receive_pmt" href="javascript:void(0)"><i class="fas fa-file-invoice-dollar" style="margin-top:3px;"></i> <span>Receive Payment</span</a>',
        //  '<a data-id="',loan_id,'" data-personid="',person_id,'" class="dropdown-item _finloan_edit" href="javascript:void(0)"><i class="fas fa-list-alt" style="color:blue;font-size:1.1em;margin-top:2px;"></i> <span>View Details</span</a>',
 
        //  '<div class="dropdown-divider"></div>',
        //  '<a data-id="',loan_id,'" data-personid="',person_id,'" class="dropdown-item _finloan_disburse" href="javascript:void(0)"><i class="fa fa-list-alt" style="color:orange"></i> Payment History</a>',
        //  '<a data-id="',loan_id,'" data-personid="',person_id,'" class="dropdown-item _finloan_create_pmt_schedule" href="#"><i class="fa fa-list" style="color:green"></i> Write off</a>',
        //  '<a data-id="',loan_id,'" data-personid="',person_id,'" class="dropdown-item _finloan_change_status" href="#"><i class="fa fa-tasks" style="color:grey;margin-top:3px;font-size:1.1em"></i> <span>Change Status</span</a>',
        //  '<a data-id="',loan_id,'" data-personid="',person_id,'" class="dropdown-item _finloan_delete" href="#"><i class="fa fa-times" style="color:red;margin-top:3px;"></i> <span>Delete Loan</span</a>',
        //  '</div>'].join('');
        // return html;
     } 
     ...
     this.init = () => {
                ...
                mThis.btnFind.on('click',(e)=>{
                    e.preventDefault();
                    mThis.displayFinishedLoans();
                });

                mThis.elSearch.on('keyup',(e)=>{
                    e.preventDefault();
                    if(e.keyCode ==13) mThis.displayFinishedLoans();
                ...
                mThis.tblLoanList.on('click','.floan_email',function(e){
                    e.preventDefault();
                    //let loan_id = $(this).data('id');
                     
                });

                mThis.tblLoanList.on('click','.floan_btn_pmt_history',function(e){
                    e.preventDefault();
                    let loan_id = $(this).data('id');
                    let p = {'start_date':null, 'end_date':null,'use_all_dates':1,'loan_id':loan_id};
                    let rpt_name ='rpt_loan_collections';
                    let params = ['rtype=',rpt_name,'&wid=','&startdate=',p.start_date,'&enddate=',p.end_date,'&usealldates:',p.use_all_dates,'&loanid=',p.loan_id].join('');
                ...
                //  mThis.tblLoanList.on('click','._finloan_delete',function(e){
                //     e.preventDefault();
     ...
     this.displayFinishedLoans = function()
     { 
         var p = {'search_value':mThis.elSearch.val()};
         post_ajax([mThis.base_url, '/api/loan/finished-loan-list'].join(''),p,function(data) {  
 ...
 //end::FinishedLoanListComponent
   
$(document).ready(function(){
    FinishedLoanListComponent.init();
});
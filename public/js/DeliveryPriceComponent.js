...
var DeliveryPriceComponent = new function(){
    ...
    this.self = $('#_sttn_deliveryPriceComponent');
...
//begin::PriceTabView
var PriceTabView = new function(){
    let mThis = this;
    this.self = $('#_sttn_priceTabView');
    this.elScreenTitle = $('#screen_title');

    this.base_url = $('#__base_url').val();
    this.elFilter_deliverytype = $('#_sttn_price_filter_dtype');
    this.elFilter_zone = $('#_sttn_price_filter_zone');
    this.elFilter_sender = $('#_sttn_price_filter_sender');
   
    this.elScriptText = $('#_sttn_price_script_text');
    this.elScriptMeaning = $('#_sttn_price_script_meaning');
    this.btnRunScript = $('#_sttn_btnRunScript');
    this.btnGetSampleScript = $('#_sttn_btnGetSample');
    this.btnImportScript = $('#_sttn_btnImportScript');
    this.elError = $('#_sttn_script_error');
   ...
   this.elFilter_sender.on('change',function(){
    ...
    this.loadFilterData = (def,onFinish)=>{
      ...
      if (mThis.form_data) {
        CommonLib.setComboItems(mThis.elFilterSender_cod,mThis.form_data.senders,'id','sender_name',false,null,def.sender_id); //Sender filter on tab view "COD FEE"
        CommonLib.setComboItems(mThis.elFilter_sender,mThis.form_data.senders,'id','sender_name',false,null,def.sender_id); //Sender filter on tab view "PRICE BY ZONE"
        CommonLib.setComboItems(mThis.elFilter_zone,mThis.form_data.zones,'zone_code','zone_name',false,null,def.zone_code); //Zone filter on tab view "PRICE BY ZONE"
        if (typeof onFinish == 'function') onFinish();
        return;
      }
       post_ajax([mThis.base_url,'/api/getFormOptions_priceline'].join(''),null,function(data){
         if(data){
           ...
           CommonLib.setComboItems(mThis.elFilterSender_cod,data.senders,'id','sender_name',false,null,def.sender_id); //Sender filter on tab view "COD FEE"
           CommonLib.setComboItems(mThis.elFilter_sender,data.senders,'id','sender_name',false,null,def.sender_id); //Sender filter on tab view "PRICE BY ZONE"
           CommonLib.setComboItems(mThis.elFilter_zone,data.zones,'zone_code','zone_name',false,null,def.zone_code); //Zone filter on tab view "PRICE BY ZONE"
    ...
    this.show = function(view_name,tab_button_clicked = false,filter_data){
       mThis.elScreenTitle.text('Manage Zone Prices');
    ...
    //begin::THIS CODE BLOCK IS NOT PART OF GENERAL SRCRIPT FOR TAB_VIEW OBJECT
           //begin::define specific elements, tables within this PriceTabView tasks 
               ...
               this.tblPrices = $('#_sttn_price_tblPrices');
               this.tblPrices_body = $('#_sttn_price_tblPrices_body');
               this.tblBaseFees = $('#_sttn_tblBaseFees');

               this.tblCODs = $('#_sttn_price_tblcods');
               this.tblCODs_body = $('#_sttn_price_tblcods_body');
               this.btnNewCOD = $('#_sttn_btnNewCOD');
               //this.elSearch_cod = $('#_sttn_cod_search');
               this.elFilterSender_cod = $('#_sttn_filter_cod_sender'); 
               this.elFilterDeliveryType_cod = $('#_sttn_filter_cod_dtype'); 

               this.tblBaseFees_cols = [];
               this.btnNewBaseFee = $('#_sttn_btnNewBaseFee');

               this.lnkAddPrice= $('#_sttn_price_lnkNewPrice');
               this.btnSaveCODCharge = $('#_sttn_btnSaveCODCharge');
               this.elCODFeeChargePercent = $('#_sttn_CODFeeChargePercent'); 

               this.elFilter_delivery_type = $('#_sttn_price_filter_dtype');  
               this.elFilter_zone= $('#_sttn_price_filter_zone');  
               this.elFilter_sender = $('#_sttn_price_filter_sender') 
               ...
               mThis.btnImportScript.on('click',(e)=>{
                   ...
                   let text = mThis.elScriptText.val();
                    let p = {'p_script':text};
                    post_ajax([mThis.base_url,'/api/importPricesByScript'].join(''),p,function(result){
               ...
               mThis.btnGetSampleScript.on('click',(e)=>{
                  let text = mThis.elScriptText.val();
                  post_ajax([mThis.base_url,'/api/getSampleScript'].join(''),null,function(d){
               ...
               mThis.btnRunScript.on('click',(e)=>{
                  let text = mThis.elScriptText.val();
                  let p = {'p_script':text};
                  post_ajax([mThis.base_url,'/api/translateScript'].join(''),p,function(d){
               ...
               this.btnSaveCODCharge.on('click',function(e){
                 ...
                 post_ajax([mThis.base_url,'/api/saveCODFeeCharge'].join(''),p,function(err){
                    if(!err || err =='') {
                        cv_interact.alert('COD Fee Charge has been saved!','','info');
              ...
              //  });
                ...
                cv_interact.confirm('Delete this Price Line?','Delete Price Line',function(e){
                      if(e){
                        let p = {'id':id,'sender_id':sender_id}; // NOTE parameter @sender_id is used to find target table "price_list" or "sender_price_list" for deleting
                         post_ajax([mThis.base_url,'/api/deletePriceLine'].join(''),p,function(err){
                            if (!err){
                               mThis.displayPrices();
                            } else cv_interact.alert(err,'','error');
                         });
               ...
               mThis.tblPrices_body.on('click','a._sttn_price_delete_city',function(e){
                 ...
                 //p.id = p.city_id;
                 cv_interact.confirm('Delete this price?','Delete Price',function(e){
                   if(e){
                     post_ajax([mThis.base_url,'/api/deletePrice'].join(''),p,function(err){
                       if(!err || err ==''){
                ...
                //  });
                  ...
                  mThis.tblBaseFees.on('click','tbody>tr button._sttn_bf_edit',function(e){
                   ...
                   mThis.tblBaseFees.on('click','button._sttn_bf_cancel',function(e){
                     ...
                     mThis.tblBaseFees.on('click','button._sttn_bf_save',function(e){
                      ...
                      if (!p) return;
                      post_ajax([mThis.base_url,'/api/saveBaseFee'].join(''),p,function(err){
                         if (!err || err=='') {
                             mThis.changeRowState(tr,'view');
                         }else cv_interact.alert(err,'','error');
                      });  
                 ...
                 mThis.tblCODs_body.on('click','a._sttn_price_cod_delete',function(e){
                   e.preventDefault();
                     let x = $(this);
                     let p = {'id':x.data('id')};
                     cv_interact.confirm('Delete COD?','Delete COD',function(e){
                       if(e) {
                          post_ajax([mThis.base_url,'/api/deleteCODFee'].join(''),p,function(err){
                            if (!err || err ==null) {
                              mThis.displayCODFees(); 
                            } cv_interact.alert(err,'','error'); 
                          });
                       }
                     });
                  });
          ...
          // }
           ...
           this.toggleZoneList = (me)=>{
            if(!me) return;
             ...
             post_ajax([mThis.base_url,'/api/getApplicableZones'].join(''),p,function(rows){
              if (rows){
           ...
           this.toggleSenderList = (me)=>{
             if(!me) return;
              ...
              post_ajax([mThis.base_url,'/api/getApplicableSenders'].join(''),p,function(rows){
                 if (rows){
                   rows = StringSanitizer.sanitizeObject(rows);
                   let i=0,c;
                   do{
         ...
         //*** begin::displayBaseFees table (Fixed Price Promotions)
          ...
          this.displayBaseFees = (onFinish=null)=>{
            post_ajax([mThis.base_url,'/api/getSenderBaseFees'].join(''),null,function(rows){
              if (typeof rows =='string') alert(rows);
                mThis.populateBaseFeesTable(rows,onFinish);
            });  
          }
        ...
        //*** end::displayBaseFees table (Fixed Price Promotions) 
         ...
         this.deleteBaseFee = (tr)=>{
           ...
           if (!id) 
              ...
              let p = {'id':id};
              cv_interact.confirm('Delete this base fee?','Delete Base Fee',function(e){
                if (e){
                   let p = {'id':id};
                   post_ajax([mThis.base_url,'/api/deleteBaseFee'].join(''),p,(err)=>{
                     if(!err || err =='') {
                        mThis.displayBaseFees();
                     } else cv_interact.alert(err,'','error');
                   });
           ...
           this.displayPrices = function(def_filter, animate = false) {
            ...
            if (animate) div.removeClass('animate-slide-left');
                   ...
                   post_ajax([mThis.base_url,'/api/getPriceList'].join(''),p,function(rows){
                       rows = StringSanitizer.sanitizeObject(rows);
                       if(rows){
                          rows = StringSanitizer.sanitizeObject(rows);
                          let i=0, c;
           ...
           this.displayCODFees = function( animate = false) {
            ...
            if (animate) div.removeClass('animate-slide-left');
                   ...
                   post_ajax([mThis.base_url,'/api/getCODFees'].join(''),p,function(rows){
                       rows = StringSanitizer.sanitizeObject(rows); 
                        if(rows){
                          rows = StringSanitizer.sanitizeObject(rows);
                          let i=0, c;
 ...
 //begin::PriceLinePanel
   var PriceLinePanel = new function(){
    this.self = $('#sttn_price_line_panel');
    let mThis = this;
    this.elError = $('#_sttn_price_error');
    this.elFilter_sender = $('#_sttn_newprice_filter_sender');
    this.elFilter_zone = $('#_sttn_newprice_filter_zone');
    this.elFilter_deliveryType = $('#_sttn_newprice_filter_delivery_type');
     
    this.elSender = $('#_sttn_newprice_filter_sender');
    this.elZone = $('#_sttn_newprice_filter_zone');
    this.elDeliveryType = $('#_sttn_newprice_filter_delivery_type');


    this.elDeliveryFee = $('#_sttn_newprice_delivery_fee');
    this.elPricePerKg = $('#_sttn_newprice_price_per_kg');
    this.elPriceOption = $('#_sttn_newprice_price_option');
    this.price_per_kg_field = $('#_sttn_price_per_kg_field');
    this.fixed_price_field = $('#_sttn_fixed_price_field');

    this.lnkBack = $('#_sttn_btnBack');
    this.btnBack1 = $('#_sttn_btnBack1');
    this.btnSavePrice = $('#_sttn_btnSavePrice');
    this.elScreenTitle = $('#screen_title');
    // let test = (e,item)=>{
    ...
    this.btnSavePrice.on('click',function(e){
      ...
      let p = mThis.getData();
      if (!p) return;
      post_ajax([mThis.base_url,'/api/savePriceLine'].join(''),p,function(d){
     ...
     //PriceLinePanel => load sender list, zone list, delivery_type
      this.loadFormOptions = (def,onFinish)=>{
        ...
        if (!mThis.sender_list || !mThis.zone_list) {
          mThis.sender_list =  $('#_sttn_newprice_sender_list').vs_multipleSelect({'onCheckChange':function(chk){
            mThis.manageChecks(mThis.sender_list, chk);
          }}); //init here because wait for vs_muitipleSelect.js to fulluy loaded

          mThis.zone_list = $('#_sttn_newprice_zone_list').vs_multipleSelect({'onCheckChange':function(chk){
        ...
        // }
         ...
         post_ajax([mThis.base_url,'/api/getFormOptions_priceline'].join(''),null,function(data){
           if(data){
                ...
                data.zones = StringSanitizer.sanitizeObject(data.zones);
                data.senders = StringSanitizer.sanitizeObject(data.senders);
                data.zones.unshift({'zone_code':'all','zone_name':'(មិនទាន់កំណត់)'});
                data.senders.unshift({'id':'all','sender_name':'(មិនទាន់កំណត់)'});
      ...
      this.displayPriceLine = (sender_id,price_id,onFinish)=>{
        ...
        post_ajax([mThis.base_url,'/api/getPriceLineData'].join(''),p,function(d){
           if (d) {
              d = StringSanitizer.sanitizeObject(d,null,['zone_codes','sender_ids']);
              mThis.setData(d);
    ...
    this.setData = (d)=>{   
      if (!d){
            ...
            mThis.elDeliveryType.val($('#_sttn_price_filter_dtype').val());  
 ...
 //begin::CODDialog
   var CODDialog = new function(){
     let mThis =this;
     this.self = $('#_sttn_cod_dlgcod');
     this.base_url = $('#__base_url').val();
     this.elTitle =$('#_sttn_cod_dlgcod_title');
     this.elTitle = $();
     mThis.sender_id = null;
     mThis.btnFindSender = $('#_sttn_cod_lnkFindSender');
     //this.id = null;
     //this.onClose;
     this.elSenderName = $('#_sttn_cod_sender_name');
     this.elCODFeePercent = $('#_sttn_cod_fee_percent');
     this.elRemarks = $('#_sttn_cod_remarks');
     this.elError = $('#_sttn_cod_error');
     this.btnOK = $('#_sttn_cod_btnOK');

     this.btnOK.on('click',function(e){
        let p = mThis.getData();
        if(!p) return;
        
        post_ajax([mThis.base_url,'/api/saveCODFeeBySender'].join(''),p,function(err){
          if(!err || err =='') {

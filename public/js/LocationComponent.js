...
//begin::CountryListPanel
 var CountryListPanel  = new function(){
    ...
    mThis.lnkNewCountry.on('click',function(e){
      ...
      InputBox1.show(option,function(d){
         if(d) {
             ...
             post_ajax([mThis.base_url,'/api/saveCountry'].join(''),p,function(err){
               if(!err || err =='') {
    ...
    mThis.tblCountries_body.on('click','a._sttn_loc_delete_country',function(e){
      ...
      if(!p.id) p.id=0;
          cv_interact.confirm('Delete this country?','Delete Country',function(e){
                if(e)
                {
                    post_ajax([mThis.base_url,'/api/deleteCountry'].join(''),p,function(err){
                      if(!err || err ==''){
                        mThis.displayCountries();
                      } else cv_interact.alert(err);
                    });
    ...
    this.displayCountries = function(){
      ...
      post_ajax([mThis.base_url,'/api/getCountryList'].join(''),null,function(rows){
         if(rows){
...
var ZoneTabView = new function(){
  ...
  this.loadComboItems_zone= (parent_zone_id,zone_name,def)=>{
    ...
    else if (zone_name =='district')
    {
     ...
     post_ajax([mThis.base_url,'/api/',method_name].join(''),p,function(rows){
       if(!rows) return;
        rows = StringSanitizer.sanitizeObject(rows);
        if (zone_name =='city') 
          {
  ...
  //begin::THIS CODE BLOCK IS NOT PART OF GENERAL SRCRIPT FOR TAB_VIEW OBJECT
         //begin::define specific elements, tables within this ZoneTabView tasks 
                 ...
                 if(!CountryListPanel.selected_country_id) {
                  ...
                  InputBox1.show(option,function(d){
                    if(d){
                       ...
                       post_ajax([mThis.base_url,'/api/saveCity'].join(''),p,function(err){
                         if(!err || err =='') {
                           mThis.displayCities(CountryListPanel.selected_country_id);
                         } else cv_interact.alert(err);
                       });
                 ...
                 if(!mThis.elFilter_city.val()) {
                  ...
                  InputBox1.show(option,function(d){
                    if(d){
                        ...
                        post_ajax([mThis.base_url,'/api/saveDistrict'].join(''),p,function(err){
                          if(!err || err =='') {
              ...
              if(!mThis.elFilter_district.val() || mThis.elFilter_district.val() <=0) {
                ...
                InputBox1.show(option,function(d){
                  if(d){
                        ...
                        post_ajax([mThis.base_url,'/api/saveCommune'].join(''),p,function(err){
                          if(!err || err =='') {
             ...
             mThis.tblCities_body.on('click','a._sttn_loc_delete_city',function(e){
               ...
               cv_interact.confirm('Delete this city?','Delete City',function(e){
                 if(e){
                   post_ajax([mThis.base_url,'/api/deleteCity'].join(''),p,function(err){
                     if(!err || err ==''){
             ...
             mThis.tblCommunes_body.on('click','a._sttn_loc_delete_commune',function(e){
              ...
              //p.id = p.city_id;
              cv_interact.confirm('Delete this commune?','Delete Commune',function(e){
                if(e){
                  post_ajax([mThis.base_url,'/api/deleteCommune'].join(''),p,function(err){
                    if(!err || err ==''){
             ...
             mThis.tblDistricts_body.on('click','a._sttn_loc_delete_district',function(e){
              ...
              cv_interact.confirm('Delete this distrinct?','Delete District',function(e){
                if(e){
                  post_ajax([mThis.base_url,'/api/deleteDistrict'].join(''),p,function(err){
                    if(!err || err ==''){
            ...
            mThis.tblCities_body.on('click','a._sttn_loc_edit_city',function(e){
              ...
              if(!country_id) {
                cv_interact.alert('No country selected!','','warning');
                return;
              }
         ...
         this.displayCities = function(country_id) {
          ...
          div.removeClass('animate-slide-left');
                 ...
                 post_ajax([mThis.base_url,'/api/getCityList'].join(''),p,function(rows){
                     rows = StringSanitizer.sanitizeObject(rows);
                     if(rows){
                        rows = StringSanitizer.sanitizeObject(rows);
                        let i=0, c;
         ...
         this.displayDistricts = function(city_id) {
             ...
             post_ajax([mThis.base_url,'/api/getDistrictList'].join(''),p,function(rows){   
               if(rows){
                rows = StringSanitizer.sanitizeObject(rows);
                let i =0, c;
                do{
         ...
         this.displayCommunes = function(district_id){
              ...
              post_ajax([mThis.base_url,'/api/getCommuneList'].join(''),p,function(rows){   
                if(rows){
                  rows = StringSanitizer.sanitizeObject(rows);
                let i =0, c;
                do{

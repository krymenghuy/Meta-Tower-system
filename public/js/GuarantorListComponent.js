...
var GuarantorsComponent = new function(){
    this.base_url = $('#__base_url').val();
    let mThis = this;
    this.self = $('#_mainGuarantorsComponent');

    this.tblGuarantors = $('#_mainGuarantorsComponent_table');

    this.btnAddGuarantorsList = $('#btnAddGuarantorList');

    this.init = () => {
        mThis.btnAddGuarantorsList.on('click', () => {
            let op = {title:'New Guarantor'};
            GuarantorsDialog.show(op, (p) => {
                //alert(JSON.stringify(p));
                post_ajax(mThis.base_url + '/api/save-applicant', p, function(results){
                    if(results.status == 'OK'){
                        mThis.displayGuarantorsList();
                    }else{
    ...
    this.displayGuarantorsList = function()
    { 
        var p = null;
        post_ajax([mThis.base_url, '/api/applicant-list'].join(''),p,function(data) {  
            ...
            if (mThis.table){
                    mThis.tblGuarantors.DataTable().clear().destroy();
                    //NOTE that ...DataTable().clear() will clear only tbody, and NOT <thead> section, so we need to ensure that the target table is cleared all, remmining only tags "<table></table>"
                    mThis.tblGuarantors.empty();
                    //alert('destroyed => '+  mThis.tblGuarantors.html());
                    mThis.table = null;
            }
            ...
            if (!mThis.table)
            mThis.table = mThis.tblGuarantors.DataTable({
                searching:false,
                destroy:true,
                paging:true,
                ordering:false,
            ...
            // $('#_dl_tblGuarantors_wrapper>div.dt-buttons').prepend(div);
                 				  
        }); //close post_ajax()
                 
    };
    //END::Display Applicant
...
var GuarantorsDialog = new function(){
    let mThis = this;
    this.self = $('#modalAddGuarantor');
    this.btnSave = $('#modalAddGuarantor_btnSave');
    this.elTitle = $('#modalAddGuarantor_title');

    this.elErrors = $('#modalAddGuarantor_error');

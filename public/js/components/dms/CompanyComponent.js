...
var CompanyComponent = new function(){
	...
	this.displayCompanyInfo = ()=>{
		vsapi.call(`${mThis.base_url}/dms/company/details`,null,null,false).then(res=>{
    ...
    this.init = ()=>{
		...
		this.btnSave.on('click',function(e){
			...
			vsapi.call(`${mThis.base_url}/dms/company/save-details`,p).then(res=>{
				if(res.status_code===200){
		 ...
		 this.btnDeleteLogo.off('click').on('click',e=>{
			   ...
			   cv_interact.confirm('Delete this logo?',{title:'Delete Logo',context: 'delete'},e=> {
				    if(e)
					{
					      vsapi.call(`${mThis.base_url}/dms/company/delete-logo`,null).then((res)=> {				 
							 if (res.status_code === 200)
							 {

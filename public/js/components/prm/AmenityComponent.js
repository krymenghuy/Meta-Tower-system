...
var AmenityComponent = (() => {
    ...
    mThis.cols = [
        ...
        
        {
            title: "Floor",
            className: "align-middle",
            data: (data) => `<span class="text-primary-custom">${data.floor ?? '-'}</span>`
        },
        // {
        //     title: "Description",
        //     className: "align-middle",
        ...
        // },
        {
            ...
            data: (data) => `<span class="text-primary-custom">${data.max_capacity ?? '-'}</span>`
        },
       {
        ...
        },
        {
            ...
            data: (data) => {
                ...
                if (status == 'inactive') {
                    ...
                    cls = 'text-white px-3 py-1 rounded-3 bg-warning d-inline-block';
                }
    ...
    mThis.init = () => {
        ...
        mThis.btnAdd.onclick = (e) => {
            ...
            const op = {
                id: null,
                btn: e.target,
                onClose: () => mThis.AmenityListView.showPage(mThis.getFilterData())
            };
        ...
        mThis.divFilter.querySelectorAll('.filter-field').forEach(el => {
            el.onchange = () => mThis.AmenityListView.showPage(mThis.getFilterData());
        });
    ...
    mThis.deleteAmenity = (id, menuLink) => {
        ...
        cv_interact.confirm('Delete this Amenity?', {
            title: 'Delete Amenity',
            context: 'delete',
            confirmButtonText: "Delete"
        }, (confirmed) => {
            if (confirmed) {
                vsapi.call(`${main_view.base_url}/prm/amenity/delete`, op, false, false, false)
                    .then(res => {
                        if (res.status_code === 200) {
                            cv_interact.success('Amenity deleted successfully');
                            mThis.AmenityListView.showPage(mThis.getFilterData());
                        } else {
                            cv_interact.error(res.error_message || 'Failed to delete amenity');
                        }
                    });
            }
        });
    ...
    mThis.changeStatus = (id, link) => {
        ...
        const inputOptions = {
            ...
            onConfirm: (status, btn, me) => {
                ...
                vsapi.post(`${mThis.base_url}/prm/amenity/update-status`, payload, { loader: false, agent: btn })
                    .then(res => {
                        if (res.status_code === 200) {
                            me.close();
                            cv_interact.success('Amenity status has been updated');
                            mThis.AmenityListView.showPage(mThis.getFilterData());
                        } else {
    ...
    mThis.prepareFormOptions = (onFinish) => {
        vsapi.call(`${main_view.base_url}/prm/amenity/form-options`, null, null,null)
            .then(res => {
                
                const d = res.status_code == 200 ? res.data : {};
                VSUtil.setComboItems(mThis.elFilter_status, d.amenity_statuses, 'id', 'name', true, 'Statuses');
    ...
    self.show = (op) => {
        dialog = dialog || new GeneralDialog({
            ...
            prepareFormOptions: {
                ...
                api: {
                    endpoint: [main_view.base_url,"/prm/amenity/form-options",].join(""),
                    params: (op) => {
                        return{ id: op.id };
                    },
                },
            ...
            buttons: [
                ...
                },
                {
                    ...
                    click: (me, btn) => {
                        ...
                        modal.querySelectorAll('input[data-field], textarea[data-field]').forEach(el => {
                        ...
                        modal.querySelectorAll('select[data-field]').forEach(el => {
                            payload[el.dataset.field] = el.value;
                        });

                        // Ensure correct numeric types
                        payload.requires_booking = Number(payload.requires_booking ?? 0);
                        payload.is_available     = Number(payload.is_available     ?? 1);
                        payload.max_capacity     = Number(payload.max_capacity     ?? 0);
                        ...
                        vsapi.call([main_view.base_url, "/prm/amenity/save"].join(""), payload, btn)
                            .then(res => {
                                if (res.status_code === 200) {
                                    ...
                                    if (me.dataOptions?.onClose) me.dataOptions.onClose();
                                } else {
                                    cv_interact.error(res.error_message || 'Failed to save amenity');
                                }
                            })
                            .catch(err => {

...
var BillComponent = (() => {
    ...
    mThis.init = () => {
        ...
        tblBill.addEventListener("click", (e) => {
            const btn = e.target.closest(".btn_upload_bill_attachment");
            if (!btn) return;
            e.preventDefault();
            const id = btn.dataset.id;
            mThis.uploadAttachment(id, btn);
        });

        tblBill.addEventListener("click", (e) => {
            const btn = e.target.closest(".btn_view_bill_attachment");
            if (!btn) return;
            e.preventDefault();
            const id = btn.dataset.id;
            mThis.viewAttachment(id, btn);
        });
    ...
    mThis.initDropdownMenus = (table) => {
        const menuOptions = {
            ...
            onShow: (me, container) => {
                ...
                if (fileUrl) {
                    menu.upload_attachment.style.display = "none";
                    menu.view_attachment.style.display = "block";
                    menu.delete_attachment.style.display = "block";
                } else {
                    menu.upload_attachment.style.display = "block";
        ...
        FileChooser.chooseFile(
            ...
            (d) => {
                ...
                const p = {
                    id: id,
                    ext: extension,
                    original_file_name: d.fileName.replace(/\.[^/.]+$/, ""),
                    data: d.dataUrl,
                    mime_type: extension,
                };

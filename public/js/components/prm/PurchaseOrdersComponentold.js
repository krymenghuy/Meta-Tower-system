...
var PurchaseOrdersComponent = (() => {
    ...
    const applyReceiveCheckboxUi = (checkboxEl) => {
        ...
        checkboxEl.style.cursor = checkboxEl.disabled
            ? "not-allowed"
            : "pointer";
    ...
    const getReceiveRoot = (me) =>
        me.divModal && me.divModal.querySelector("#receive_purchase_item_list");
    const getReceiveItems = (me) =>
        me.receiveItemsView &&
        typeof me.receiveItemsView.getItems === "function"
            ? me.receiveItemsView.getItems() || []
            : [];
    const getReceiveQtyRaw = (rowOrItem) => {
        if (!rowOrItem) return undefined;
        return rowOrItem.receive_qty != null
            ? rowOrItem.receive_qty
            : rowOrItem.recieve_amount;
    };
    ...
    const mapReceiveRows = (items) =>
        (items || []).map((it) => {
            ...
            return {
                ...
                total_price:
                    it.total_price ||
                    Number(it.qty) * Number(it.unit_price || 0),
                receive_qty: receiveRaw != null ? Number(receiveRaw) : 0,
                break_amount:
                    it.break_amount != null ? Number(it.break_amount) : 0,
                code: it.code || "",
            };
        });
    ...
    const applyPoHeaderToDialog = (me, poDetails, vendors, withCode) => {
        ...
        if (me.controls.po_date)
            me.controls.po_date.value = poDetails.po_date || "";
        if (me.controls.po_number)
            me.controls.po_number.value = poDetails.po_number || "";
        if (me.controls.discount_value)
            me.controls.discount_value.value = poDetails.discount_value ?? 0;
        if (me.controls.discount_type)
            me.controls.discount_type.value =
                poDetails.discount_type || "percent";
    ...
    const poTotalsBoxHtml = (pfx, dis) => {
        ...
        return (
            `<div class="col-lg-12 mt-3 d-flex justify-content-end"><div class="p-3 rounded-3 shadow-sm border" style="background-color:#fff; min-width:280px;">` +
            poFormRow(
                "Sub Total",
                `<span id="${pfx}subtotal_display" class="ms-2">$ 0.00</span>`,
                true,
            ) +
            poFormRow("Discount", dInp + dSel, true) +
            poFormRow(
                "Tax",
                `<span id="${pfx}tax_display" class="ms-2">$ 0.00</span>`,
                true,
            ) +
            poFormRow(
                "Total",
                `<span id="${pfx}total_display" class="ms-2 fw-bold">$ 0.00</span>`,
                true,
            ) +
            "</div></div>"
        );
    ...
    const calcTotals = (subTotal, discountVal, discountType) => {
        ...
        const dt =
            (discountType || "percent") === "percent" ? "percent" : "amount";
    ...
    const removeLeadingEmptyItemRow = (view) => {
        ...
        if (
            typeof view.getItems === "function" &&
            typeof view.setData === "function"
        ) {
    ...
    const applyUnitOptions = (itemsView) => {
        if (!itemsView || typeof itemsView.setSelectOptions !== "function")
            return;
        itemsView.setSelectOptions("unit", UNIT_OPTIONS, "", {
            value: "id",
            label: "Select unit",
        });
    };
    ...
    const computeReceiveProgressStateFromItems = (items) => {
        ...
        rows.forEach((it) => {
            ...
            const recv =
                recvRaw != null && !Number.isNaN(Number(recvRaw))
                    ? Number(recvRaw)
                    : 0;
        ...
        meaningful.forEach((m) => {
            ...
            if (!m.lineMarkedReceived && m.eff + 1e-6 < m.ordered)
                allSatisfied = false;
        });
    ...
    const updateReceiveProgressBadge = (me) => {
        const el =
            me.divModal &&
            me.divModal.querySelector("#receive_po_progress_badge");
        if (!el) return;
        ...
        const cfg =
            RECEIVE_PROGRESS_BADGE_MAP[state] ||
            RECEIVE_PROGRESS_BADGE_MAP.none;
        el.textContent = cfg.label;
        el.className = `${cfg.cls} text-capitalize d-inline-block text-center`;
        el.setAttribute("style", cfg.style);
    };
    ...
    const getCleanPurchaseItems = (itemsView) => {
        ...
        return source
            ...
            .map((row) => ({
                ...row,
                trx_id:
                    Number(row?.trx_id ?? 0) > 0 ? Number(row.trx_id) : null,
            }));
    };
    const applyReceiveActionColumn = (me, rows) => {
        ...
        const isDeleteCell = (cell) => {
            ...
            return !!cell.querySelector(
                "i.fa-trash, i.fa-trash-can, i.fa-trash-alt, .fa-regular.fa-trash-can, .fa-solid.fa-trash-can",
            );
        };
        theadRow
            .querySelectorAll("th[data-receive-po-col]")
            .forEach((el) => el.remove());
        tbody
            .querySelectorAll("td[data-receive-po-col]")
            .forEach((el) => el.remove());

        theadRow
            .querySelectorAll("th.iv-th-action")
            .forEach((el) => el.remove());
        ...
        tbody.querySelectorAll("tr").forEach((tr, idx) => {
            const existingActionCell = Array.from(
                tr.querySelectorAll("td"),
            ).find((cell) => isDeleteCell(cell));
            ...
            while (cells.length > expectedCellCount) {
                const customActionCell = tr.querySelector(
                    "td[data-receive-po-col]",
                );
                const removable =
                    customActionCell &&
                    customActionCell.previousElementSibling &&
                    customActionCell.previousElementSibling.tagName === "TD"
                        ? customActionCell.previousElementSibling
                        : cells
                              .slice()
                              .reverse()
                              .find(
                                  (cell) =>
                                      !cell.hasAttribute("data-receive-po-col"),
                              );
                if (!removable) break;
                removable.remove();
                cells = Array.from(tr.children).filter(
                    (el) => el.tagName === "TD",
                );
            }
        });
    ...
    const refreshReceiveQtyDependentUi = (me) => {
        if (typeof me.updateReceiveTotals === "function")
            me.updateReceiveTotals();
        refreshReceiveLineActionsInPlace(me, getReceiveItems(me));
        scheduleReceiveProgressBadgeUpdate(me);
    };
    const RECV_QTY_INPUT_SEL = [
        'input[data-field="receive_qty"]',
        'input[name="receive_qty"]',
        'input[data-field="recieve_amount"]',
        'input[name="recieve_amount"]',
        'input[data-name="receive_qty"]',
        'input[data-name="recieve_amount"]',
    ];
    const getReceiveQtyCellInRow = (tr) => {
        ...
        const isRecvInp = (el) => {
            ...
            return ["receive_qty", "recieve_amount"].some(
                (k) =>
                    nm === k ||
                    df === k ||
                    dn === k ||
                    (id && String(id).includes(k)),
            );
        };
        ...
        const ths = tr
            ...
            ?.querySelectorAll?.("th");
        if (!ths) return null;
        const idx = Array.from(ths).findIndex(
            (th) =>
                String(th.textContent || "")
                    .trim()
                    .toLowerCase() === "received qty",
        );
    ...
    const refreshReceivePoHeaderStatus = (me) => {
        ...
        return vsapi
            .call(
                `${main_view.base_url}/prm/purchase/order/form-options`,
                { id: poId },
                null,
                false,
            )
    ...
    const showIncompleteReceiveRemarkDialog = ({
        poId,
        triggerEl,
        onConfirmed,
    }) => {
        let incompleteReceiveDialog = mThis._incompleteReceiveDialog;
        incompleteReceiveDialog =
            incompleteReceiveDialog ||
            new GeneralDialog({
            ...
            `,
                onShow: (dlg) => {
                    const titleEl =
                        dlg.divModal &&
                        dlg.divModal.querySelector(".modal-title");
                    if (titleEl)
                        titleEl.innerHTML =
                            '<h2 class="text-prm-custom text-start fw-bold">Confirm Receive</h2>';
                    ...
                    },
                    {
                        ...
                        click: (dlg, btn) => {
                            const remarks = String(
                                dlg.controls.remarks?.value || "",
                            ).trim();
                            if (!remarks)
                                return cv_interact.warning(
                                    "Please enter remarks.",
                                );
                            if (typeof onConfirmed === "function")
                                onConfirmed({ remarks, btn, dlg });
                        },
                    },
                ],
            });
    ...
    const getReceiveLineRowData = (me, poItemId, tr) => {
        ...
        const orderedQty =
            row && row.qty != null && !Number.isNaN(Number(row.qty))
                ? Number(row.qty)
                : 0;
        const receiveRaw = row && getReceiveQtyRaw(row);
        const receiveQty =
            receiveRaw != null && !Number.isNaN(Number(receiveRaw))
                ? Number(receiveRaw)
                : 0;
        const breakAmount =
            row &&
            row.break_amount != null &&
            !Number.isNaN(Number(row.break_amount))
    ...
    const resetLineCheckbox = (tr) => {
        const cb =
            tr && tr.querySelector
                ? tr.querySelector(".receive-po-line-cb")
                : null;
        if (cb) cb.checked = false;
    };
    const refreshReceiveActions = (me) =>
        refreshReceiveLineActionsInPlace(me, getReceiveItems(me));
    /**
    ...
    const submitReceivePoLine = (me, tr, poItemId, loadingEl, opts = {}) => {
        ...
        const { orderedQty, receiveQty, breakAmount } = getReceiveLineRowData(
            me,
            poItemId,
            tr,
        );
        ...
        if (rq + ba > oq) {
            clearCb();
            cv_interact.error(
                "Receive qty + break amount cannot exceed ordered qty.",
            );
            return Promise.resolve(false);
        }
        ...
        return vsapi
            .call(
                `${main_view.base_url}/prm/purchase/order/receive`,
                {
                    id: poId,
                ...
                loadingEl || tr.querySelector(".receive-po-line-cb") || tr,
            )
            .then((res) => {
                ...
                cv_interact.warning(
                    res.error_message || "Could not receive line.",
                );
                return false;
            })
            .catch(() => {
    ...
    const saveCheckedReceiveLines = (me, triggerEl) => {
        ...
        const checked = Array.from(
            root.querySelectorAll("tbody .receive-po-line-cb:checked"),
        ...
        return checked
            .reduce((chain, cb) => {
                return chain.then((ok) => {
                    ...
                    return submitReceivePoLine(me, tr, poItemId, triggerEl, {
                        silent: true,
                        skipReload: true,
                    });
                });
    ...
    const bindReceiveLineActionDelegation = (me) => {
        ...
        root.addEventListener("change", (e) => {
            const cb =
                e.target &&
                e.target.closest &&
                e.target.closest(".receive-po-line-cb");
            ...
            if (cb.checked) {
                ...
                if (Number(receiveQty) <= 0) {
                    ...
                    setReceiveQtyLocked(tr, false);
                    return cv_interact.warning(
                        "Enter received qty before selecting a line.",
                    );
                }
            }
            setReceiveQtyLocked(tr, cb.checked);
        ...
        if (itemRoot) {
            itemRoot
                ...
                .forEach((el) => {
                    if (
                        el.classList &&
                        el.classList.contains("receive-po-line-cb")
                    )
                        return;
                    const nm = el.getAttribute("name") || "";
                    const df =
                        el.dataset && el.dataset.field ? el.dataset.field : "";
                    if (
                        nm === "receive_qty" ||
                        df === "receive_qty" ||
                        nm === "recieve_amount"
    ...
    const loadReceivePurchaseOrder = (me) => {
        ...
        if (
            !me.receiveItemsView ||
            typeof me.receiveItemsView.setData !== "function"
        ) {
            return Promise.reject(
                new Error("Receive items view is not initialized"),
            );
        }
        return Promise.all([
            vsapi.call(
            ...
            vsapi.call(
                `${main_view.base_url}/prm/purchase/order/items-by-po`,
                { id: poId, po_id: poId },
                null,
                false,
            ),
            ...
            if (me._itemOptions && me.receiveItemsView?.setSelectOptions) {
                me.receiveItemsView.setSelectOptions(
                    "item_id",
                    mapItemSelectOptions(me._itemOptions),
                    null,
                );
            }
    ...
    mThis.cols = [
        { title: "", className: "align-middle" },
        {
            ...
            data: (data) =>
                `<span class="text-nowrap text-prm-custom">${data.po_number ?? ""}</span>`,
        },
        {
            transTitle: "titles.Vendor",
            className: "align-middle text-nowrap",
            data: (data) =>
                `<span class="d-block text-prm-custom">${data.vendor_name}</span>`,
        },
        {
            title: "Po Date",
            className: "align-middle text-nowrap",
        ...
        },
        {
            ...
            data: (data) =>
                `<span class="d-block text-prm-custom">${data.sub_total_formatted}</span>`,
        },
        {
            title: "Discount",
            className: "align-middle text-nowrap text-end",
            data: (data) =>
                `<span class="d-block text-prm-custom">${data.discount_formatted}</span>`,
        },
        {
            title: "Total Amount",
            className: "align-middle text-nowrap text-end",
            data: (data) =>
                `<span class="d-block text-prm-custom">${data.total_amount_formatted}</span>`,
        },
        {
            title: "Authorized By",
    ...
    const resolvePoIdFromMenuContext = (id, menuLink) => {
        let poId = id;
        const btn =
            typeof menuLink === "object" && menuLink?.target
                ? menuLink.target
                : menuLink;
        const el = btn && btn.closest ? btn.closest("[data-id]") : null;
        if ((!poId || Number(poId) <= 0) && el && el.dataset && el.dataset.id)
            poId = el.dataset.id;
    ...
    const renderPoItem = (po, container, onFinish) => {
        ...
        if (!poId) {
            container.innerHTML =
                '<div class="alert alert-warning m-3">No purchase order selected.</div>';
            return;
        }
        container.innerHTML =
            '<div class="text-center py-3"><div class="spinner-border text-primary" role="status"></div><div class="small text-muted mt-1">Loading items...</div></div>';
        vsapi
            .call(
                `${main_view.base_url}/prm/purchase/order/items-by-po`,
                { id: poId, po_id: poId },
            ...
            .then((res) => {
                ...
                const rows = items
                    ...
                    <tr>
                        ...
                        <td class="text-nowrap">${item.unit_price_formatted}</td>
                        <td class="text-nowrap">${item.total_price_formatted}</td>
                    </tr>
                `,
                    )
                    .join("");
            ...
            .catch(() => {
                container.innerHTML =
                    '<div class="alert alert-danger m-3">Failed to load purchase order items.</div>';
            });
    };
            ...
            cv_interact.confirm(
                ...
                (e) => {
                    if (e) {
                        vsapi
                            .call(
                                `${main_view.base_url}/prm/purchase/order/authorized`,
                                op,
                                false,
                                null,
                                null,
                            ...
                            .then((res) => {
                                if (res.status_code === 200) {
                                    ...
                                    cv_interact.success(
                                        "Purchase Order has been authorized!",
                                    );
        ...
        cv_interact.confirm(
            ...
            function (e) {
                if (e) {
                    vsapi
                        .call(
                            `${main_view.base_url}/prm/purchase/order/delete`,
                            { id: id },
                            false,
                            false,
                            false,
    ...
    const showPurchaseOrderDialog = (op) => {
        const loadPurchaseOrderForEdit = (me, editPoId) => {
            ...
            if (
                !me.purchaseItemsView ||
                typeof me.purchaseItemsView.setData !== "function"
            ) {
                return Promise.reject(
                    new Error("Purchase items view is not initialized"),
                );
            }
            ...
            return vsapi
                ...
                .then((formRes) => {
                    ...
                    if (titleEl2)
                        titleEl2.innerHTML =
                            '<h2 class="text-prm-custom text-start fw-bold">Modify Purchase Order1</h2>';
                ...
                .then((itemsRes) => {
                    ...
                    if (typeof me.updatePOTotals === "function")
                        me.updatePOTotals();
                    if (!items.length) {
                        cv_interact.warning(
                            "This purchase order has no items (items-by-po returned empty).",
                        );
                    }
                    return items;
                });
            ...
            new GeneralDialog({
                ...
                override: {
                    ...
                    setData: (dlg, data) => {
                        ...
                        rootEl.querySelectorAll(".data-input").forEach((el) => {
                            if (el.closest && el.closest("#purchase_item_list"))
                                return;
                            const field =
                                el.dataset?.field || el.getAttribute("name");
                            if (!field) return;
                            const val = data[field] ?? "";
                            if (el.tagName === "SELECT") {
                                el.value = val;
                ...
                contentCreated: (me) => {
                    ...
                    me.purchaseItemsView = new ItemsView("purchase_item_list", {
                        ...
                        onItemChange: async (
                            ...
                            if (!tr || tr.dataset.poItemDetailHydrate === "1")
                                return;
                            ...
                            const res = await vsapi.call(
                                `${main_view.base_url}/prm/item/details`,
                                {
                                    item_id: itemId,
                                    vendor_id:
                                        me.dataOptions.vendor_id ||
                                        me.dataOptions.owner_id,
                                },
                                false,
                            );
                            ...
                            try {
                                ...
                                me.purchaseItemsView.setCellValue(
                                    tr,
                                    "item_id",
                                    itemId,
                                );
                                const itemSelectEl = tr.querySelector(
                                    '[name="item_id"], [data-field="item_id"]',
                                );
                                if (
                                    itemSelectEl &&
                                    itemSelectEl.tagName === "SELECT"
                                ) {
                                    let hasOption = false;
                                    for (
                                        let i = 0;
                                        i < itemSelectEl.options.length;
                                        i++
                                    ) {
                                        if (
                                            String(
                                                itemSelectEl.options[i].value,
                                            ) === String(itemId)
                                        ) {
                                            hasOption = true;
                                            break;
                                ...
                                if (
                                    itemDetails.unit_id != null ||
                                    itemDetails.unit != null
                                ) {
                                    me.purchaseItemsView.setCellValue(
                                        tr,
                                        "unit",
                    ...
                    const itemListEl = me.divModal.querySelector(
                        "#purchase_item_list",
                    );
                    if (itemListEl) {
                        itemListEl.addEventListener("input", () => {
                            if (typeof me.updatePOTotals === "function")
                                me.updatePOTotals();
                        });
                        itemListEl.addEventListener("change", () => {
                            if (typeof me.updatePOTotals === "function")
                                me.updatePOTotals();
                        });
                    }
                    ...
                    me.updatePOTotals = () => {
                        ...
                        const items = getCleanPurchaseItems(
                            me.purchaseItemsView,
                        );
                        let subTotal = 0;
                    ...
                    me.clear = () => {
                        for (const name in me.fields) {
                            ...
                            if (
                                ["SELECT", "INPUT", "TEXTAREA"].indexOf(tag) >=
                                0
                            ) {
                                el.value = "";
                            } else {
                        ...
                        if (me.controls.discount_value)
                            me.controls.discount_value.value = "0";
                        if (me.controls.discount_type)
                            me.controls.discount_type.value = "percent";
                        me.purchaseItemsView.setData(null);
                        me.updatePOTotals();
                    };
                    bindTotalsInputListeners(
                        me,
                    ...
                    const tryLoad = (attempt = 0) => {
                        if (
                            ...
                            return cv_interact.error(
                                "Purchase items view not ready.",
                            );
                        }
                        loadPurchaseOrderForEdit(me, editPoId).catch((err) => {
                            cv_interact.error(
                                err?.message ||
                                    "Failed to load purchase order for edit.",
                            );
                        });
                    };
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        click: (me) => {
                            ...
                            if (
                                !me.purchaseItemsView ||
                                typeof me.purchaseItemsView.getItems !==
                                    "function"
                            ) {
                                return cv_interact.error(
                                    "Purchase items are not ready. Please try again.",
                                );
                            }
                            p.items = getCleanPurchaseItems(
                                me.purchaseItemsView,
                            );
                            if (!hasValidPurchaseOrderLineItems(p.items)) {
                                return cv_interact.error(
                                    "Please select at least one item before saving the purchase order.",
                                );
                            }
                            ...
                            vsapi
                                .call(
                                    `${main_view.base_url}/prm/purchase/order/save`,
                                    p,
                                    false,
                                )
                                .then((res) => {
        ...
        if (op.id) {
            setTimeout(() => {
                const t =
                    PurchaseOrderDialog.divModal &&
                    PurchaseOrderDialog.divModal.querySelector(".modal-title");
                if (t)
                    t.innerHTML =
    ...
    const showReceivePurchaseOrderDialog = (op) => {
        ReceivePurchaseOrderDialog =
            ...
            new GeneralDialog({
                ...
                override: {
                    setData: (dlg, data) =>
                        setDialogDataExcluding(
                            dlg,
                            data,
                            "#receive_purchase_item_list",
                        ),
                },
                createContent: () => `
            <div class="row mb-4">
            ...
            `,
                contentCreated: (me) => {
                    me.receiveItemsView = new ItemsView(
                        "receive_purchase_item_list",
                        {
                            columns: [
                                {
                    ...
                    me.updateReceiveTotals = () => {
                        ...
                        const items = me.receiveItemsView.getItems
                            ? me.receiveItemsView.getItems()
                            : [];
                        let subTotal = 0;
                        if (Array.isArray(items)) {
                ...
                onShow: (me) => {
                    ...
                    const loadingEl = me.divModal.querySelector(
                        "#receive_po_loading",
                    );
                    const wrap = me.divModal.querySelector(
                        "#receive_po_content_wrap",
                    );
                    ...
                    const tryLoad = (attempt = 0) => {
                        if (
                            ...
                            return cv_interact.error(
                                "Receive items view is not ready.",
                            );
                        }
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        click: (me, btn) => {
                            ...
                            if (
                                Number(me._receivePoStatusId) ===
                                PO_HEADER_STATUS.RECEIVED
                            ) {
                                return cv_interact.warning(
                                    "This purchase order is already fully received.",
                                );
                            ...
                            saveCheckedReceiveLines(me, btn).then((savedOk) => {
                                ...
                                if (
                                    ...
                                    cv_interact.success(
                                        "Purchase order confirmed as received.",
                                    );
                                    me.hide(true);
                                    if (
                                        mThis.PoListView &&
                                ...
                                if (!validateAllReceiveLinesComplete(me)) {
                                    return showIncompleteReceiveRemarkDialog({
                                        ...
                                        onConfirmed: ({
                                            ...
                                            vsapi
                                                .call(
                                                    `${main_view.base_url}/prm/purchase/order/confirm-received`,
                                                    {
                                                        id: poId,
                                                        allow_partial: 1,
                                                        remarks,
                                ...
                                vsapi
                                    ...
                                    .then((res) => {
                                        if (res.status_code === 200) {
                                            ...
                                            cv_interact.warning(
                                                res.error_message ||
                                                    "Could not confirm purchase order.",
                                            );
                                        }
                                    });
                            });

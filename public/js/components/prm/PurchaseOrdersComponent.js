...
var PurchaseOrdersComponent = (() => {
    ...
    mThis.cols = [
        ...
        // },
        {
            ...
            data: (data) => {
                const cls_color =
                    data.total_amount > 0 ? "text-prm-custom" : "text-danger";
                const total_amount = VSMoney.formatAmount(
                    data.total_amount,
        ...
        new ExpandableRowConfig(tblPo.id, {
            ...
            onOpen: (container, detail_tr, parent_tr) => {
                renderPurchaseOrderItems(
                    { po_id: parent_tr.dataset.id },
                    detail_tr.querySelector(".expandable-row-container"),
                );
            },
        });
        ...
        const menuOptions = {
            ...
            onShow: (me, container) => {
                ...
                if (status_id == 1)
                    ...
                    allowed = ["receive_purchase_order"];
                else if (status_id == 6 || status_id == 7)
                    allowed = ["delete_purchase_order"];
                else if (status_id == 5) allowed = ["delete_purchase_order"];

                Object.keys(menu).forEach((key) => {
                    if (menu[key]?.style) {
                        menu[key].style.display = allowed.includes(
                            menu[key].dataset.mnuaction,
                        )
                            ? "block"
                            : "none";
                    }
        ...
        Swal.fire({
            ...
            inputValidator: (value) => {
                if (!value) return "Remark required!";
                    ...
                    vsapi
                        .call(
                            `${main_view.base_url}/prm/purchase/order/reject`,
                            op,
                            null,
                        )
                        .then((res) => {
        ...
        PurchaseOrderDialog =
            ...
            new GeneralDialog({
                ...
                contentCreated: (me) => {
                    ...
                    me.controls.div_purchase_summary =
                    ...
                    me.searchVendor = VSSearchInput.init(me.controls.vendor, {
                        ...
                        processResponse: (res) => {
                            ...
                            return (Array.isArray(vendors) ? vendors : []).map(
                                (v) => ({
                                    ...v,
                                    vendor:
                                        v.vendor ||
                    ...
                    me.purchaseItemsView = new ItemsView(
                        me.controls.purchaseItemList,
                        {
                            currencyCode: "USD",
                            ...
                            totalSummary: {
                                container: me.controls.div_purchase_summary,
                                showTax: false,
                                allowDiscount: false,
                            ...
                            addLineButtonText: LocaleManager.trans(
                                "Add Item",
                                "buttons",
                            ),
                            ...
                            beforeAddRow: async (itemsView, ctx) => {
                                const rows = itemsView.rows || [];
                                if (rows.length > 0) {
                                    const lastRowInGrid = rows[rows.length - 1];
                                    ...
                                    if (lastRowData.unit_price === undefined || lastRowData.unit_price === null || lastRowData.unit_price === "") {
                                        cv_interact.error(LocaleManager.trans("please_enter_price"));
                                        return false;
                                    }
                                }


                                const lastTr = ctx.lastTr;
                                ...
                                if (lastTr.dataset.rowState === 'edit') {
                                    const valid = itemsView.validateRow(
                                        lastTr,
                                        {
                                            zone_code: 'string',
                    ...
                    // const origSetData = me.purchaseItemsView.setData;
                    // me.purchaseItemsView.setData = function(data, options) {
                    //     origSetData.call(this, data, { ensureEmptyRow: false, ...options });
                    // };
                    ...
                    me.saveData = (onFinish) => {
                        ...
                        let totals =
                            me.purchaseItemsView.getCurrentTotals?.() || {};
                    ...
                    //     });
                    // };
                    me.controls.purchaseItemList.addEventListener(
                        "input",
                        (e) => {
                            const target = e.target;
                            if (!target.closest('td[data-name="unit_price"]'))
                                return;
                ...
                onPrepareForm: (me, data) => {
                    ...
                    if (me.dataOptions.id) {
                        ...
                        me.controls.vendor.value =
                            po.name || po.vendor_name || "";
                        if (po.vendor_id) {
                            me._selectedVendorId = po.vendor_id;
                            if (me.controls.vendor_id)
                                me.controls.vendor_id.value = po.vendor_id;
                    ...
                    me.purchaseItemsView = new ItemsView(
                        me.controls.purchaseItemList,
                        {
                            ...
                            itemRendered: (iMe, ctx) => {
                                ...
                                const checkbox =
                                    tr.querySelector(".check_accept");
                                const cell = tr.querySelector(
                                    "[data-name='received_qty']",
                                );
    ...
    mThis.prepareFormOptions = (onFinish) => {
        vsapi
            .call(`${main_view.base_url}/prm/purchase/order/form-options`)
            .then((res) => {

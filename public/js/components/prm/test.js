...
const InvoiceDialog = (() => {
    ...
    self.show = (op = {}) => {
        dlg = dlg || new GeneralDialog({
            ...
            contentCreated: (me) => {
                // Map all form inputs into me.controls by data-field or name
                ...
                me.itemsView = new ItemsView(me.controls.divItemsView, {
                    columns: [
                        ...
                        { name: "Type",      transTitle: "titles.Type",        dataType: "text"
                    ...
                    // When user picks a product → auto-fill price and remarks
                    onItemChange: (rowId, item, fieldName, td, tr) => {
                        if (fieldName !== "item_id") return;
                        ...
                        me.itemsView.setCellValue(tr, "remarks", service.service || service.name || "—");
                    }
                });
                ...
                me.populateServiceDropdown = function(retries = 3) {
                    ...
                    if      (typeof iv.setColumnOptions    === 'function') { iv.setColumnOptions('item_id', options);    return; }
                    else if (typeof iv.setSelectOptions    === 'function') { iv.setSelectOptions('item_id', options);    return; }
                    else if (typeof iv.setOptionsForColumn === 'function') { iv.setOptionsForColumn('item_id', options); return; }

                    // Fallback: update <select> directly in the DOM (retry if not rendered yet)
                    setTimeout(() => {
                ...
                // Tenant live-search: on select → fill phone, email, space dropdown
                me.searchTenant = VSSearchInput.init(me.controls.tenant, {
                    type: 'select',
                    prefetch: true,
                    query: {
                    ...
                    columns: { name: "Name", phone_number: "Phone" },
                    onSelect: (tenant) => {
                        me._selectedTenantId = tenant.id;   // store numeric ID for saving
                        vsapi.post(`${main_view.base_url}/prm/tenant/options-tenant-info`, { tenant_id: tenant.id }, {})
                            .then(res => {
                                const d = res.data || {};
                                me.controls.phone_number.value = d.tenant?.phone_number || '';
                ...
                // ================================================================
                //  saveData — maps frontend field names → backend field names
                //
                //  Problem                    Frontend sends    Backend expects
                //  ──────────────────────────────────────────────────────────────
                //  Tenant stored as text    →  tenant_id text  →  numeric ID
                //  Item text label          →  remarks         →  description
                //  Item calculated price    →  price + qty     →  amount
                //  Product foreign key      →  item_id         →  service_id
                // ================================================================
                me.saveData = () => {
                    ...
                    // Fix 1: replace text value with the real numeric tenant ID
                    if (me._selectedTenantId) {
                        header.tenant_id = me._selectedTenantId;
                    }

                    // Fix 2: remap each item's fields to match backend expectations
                    const mappedItems = items.map(item => {
                        const qty    = parseFloat(item.qty   ?? 1);
                        const price  = parseFloat(item.price ?? 0);
                        const amount = qty * price;   // backend validates item.amount, not qty+price

                        return {
                            ...item,
                            description : item.remarks || item.description || '',  // backend: 'description'
                            amount      : amount,                                   // backend: 'amount'
                            service_id  : item.item_id || null,                    // backend: 'service_id'
                        };
                    });

                    return { ...header, items: mappedItems, ...totals };
                };
            },
            ...
            onPrepareForm: (me, data) => {
                ...
                // Re-populate dropdown now that serviceList is loaded
                setTimeout(() => {
                ...
                },
                {
                    ...
                    click:    (me, btn) => {
                        ...
                        vsapi.call(`${main_view.base_url}/prm/invoice/save`, formData, btn)
                            .then(res => {
                                if (res.status_code === 200) {
                                    cv_interact.success(formData.id ? 'Invoice updated.' : 'Invoice created.');
                                    me.hide(true);
                                } else {
                                    cv_interact.error(res.error_message || 'Save failed.');
                                }

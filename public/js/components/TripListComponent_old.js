...
var TripListComponent = new function() {
    ...
    this.packageStatusChanged_eventHandler = (d)=>{
        let data = d.data;
        let title = data.title?data.title:'Package Status'; 
        if (data.status_id ==9)
    ...
    this.init = function() {        
        ...
        mThis.tblTrips.on('click','tbody>tr.trip_header',function(e){
            ...
            if(!btn_trip_action.is(e.target) && btn_trip_action.has(e.target).length ===0) {
                if (!dropdown_menu.is(e.target) && dropdown_menu.has(e.target).length ===0)
                    mThis.expanded_detail.displayPackageList(tr,did,false); 
            }
        });
        ...
        mThis.tblTrips.on('click','tbody>tr.package_list a.trl_copy_barcode',function(e){
            e.preventDefault();
            ...
            PackageStatusDialog.show(op,(d)=>{
                if(d){
                    ...
                    vsapi.call([mThis.base_url,'/api/updatePackageStatus'].join(''),p).then(res=>{
                        if(res.status_code ===200){ 
                            mThis.expanded_detail.refreshPackageList(tr,tr.prev());
                        }
                        else cv_interact.error(res.error_message);
                    });
                }
    ...
    this.displayDeliveryTrips = function(filter,search_action = false,onFinish=null){   
        ...
        vsapi.call([mThis.base_url, '/api/getDeliveryTrips'].join(''),p).then(res=>{  
            if(mThis.table){
                mThis.tblTrips.DataTable().clear().destroy();
                mThis.tblTrips.empty();
                mThis.table = null;
            }
            //NOTE that "expanded_detail" is a sub class of TripListComponent class
            //clear Trip list's detail pacakge list row (tr) to avoid misbehavior of Expandable Package List row defined as class named "expanded_detail"
            mThis.expanded_detail.prev_package_detail_tr = null;

            let data = [];
            if(res.status_code ===200) data = StringSanitizer.sanitizeObject(res.data);
                ...
                },
                {
                    ...
                    data:function(data,a,b){
                        return ['<div style="display:flex;flex-direction:column">',
                            '<div><span class="total-value trip-driver-total">',data.driver_total?data.driver_total:0,'</span></div>',
                        '</div>'].join(''); 
                    },
                    title:'Total'
                },
    ...
    this.addPackageToTrip = (p,closeOnSuccess=false) => {
        vsapi.call([mThis.base_url,'/api/addPackageToTrip'].join(''),p).then(res=>{
            ...
            if(res.status_code ===200) {
                ...
                TripListComponent.expanded_detail.refreshPackageList(list_tr);

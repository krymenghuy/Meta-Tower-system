        ...
        dialog = new GeneralDialog({
            ...
            contentCreated: (me) => {
                const updatePricePreview = () => {
                    ...
                    const durationRow =
                        me.divModal.querySelector(".select-type-time");
                    const previewRow =
                        me.divModal.querySelector("#price-preview-row");
                    const unitWrapper =
                        me.divModal.querySelector(".unit-type-wrapper");

                    if (unitWrapper) {
                        unitWrapper.classList.remove("col-md-3", "col-md-6");
                        unitWrapper.classList.add(
                            showDuration ? "col-md-3" : "col-md-6",
                        );
                    }
                    if (durationRow) {
                    ...
                    const hours = parseFloat(
                        me.controls.duration_hours?.value || 0,
                    );
                    const price = parseFloat(me.servicePrice || 0);
                    if (hours > 0 && price > 0) {
                        const total = price * hours;
                        me.divModal.querySelector("#calc-total").textContent =
                            `$${total.toFixed(2)}`;
                        me.divModal.querySelector(
                            "#calc-breakdown",
                        ).textContent = `$${price.toFixed(2)} × ${hours}h`;
                        if (previewRow) {
                ...
                me.loadTenantOptions = (sourceData, restoreValues = null) => {
                    const d = sourceData || {};
                    me._availableServices = d.service || [];

                    // console.log(23456, d)

                    // Populate combo fields directly from the dataset payload
                    VSUtil.setComboItems(
                    ...
                    if (restoreValues) {
                        if (restoreValues.space_id)
                            me.controls.space_id.value = String(
                                restoreValues.space_id,
                            );

                        // let resolvedCategoryId = restoreValues.category_id;
                        // if (!resolvedCategoryId && restoreValues.service_id) {
                        //     const match = me._availableServices.find(
                        //         (s) =>
                        //             String(s.id) ===
                        //             String(restoreValues.service_id),
                        //     );
                        //     if (match) resolvedCategoryId = match.category_id;
                        // }
                        if (resolvedCategoryId)
                            me.controls.category_id.value =
                        ...
                        if (restoreValues.service_id)
                            me.controls.service_id.value = String(
                                restoreValues.service_id,
                            );
                        ...
                        if (me.controls.unit_type && restoreValues.unit_type)
                            me.controls.unit_type.value =
                                restoreValues.unit_type;
                        if (
                            me.controls.duration_hours &&
                            restoreValues.duration_hours
                        )
                            me.controls.duration_hours.value =
                                restoreValues.duration_hours;
                    } else {
                        me.controls.service_id.innerHTML =
                ...
                ["unit_type", "duration_hours"].forEach((f) => {
                    me.controls[f]?.addEventListener(
                        "change",
                        updatePricePreview,
                    );
                });
            ...
            onPrepareForm: (me, data) => {
                ...
                const detail = data.request_details || {};
                ...
                if (data.request_details) {
                    ...
                    // Edit/Modify Mode: Render drops and assign initial states
                    me.loadTenantOptions(data, {
                        space_id: detail.space_id,
                        category_id: detail.category_id,
                        service_id: detail.service_id,
                        unit_type: detail.unit_type,
                    ...
                    // Create Mode: Render dropdown selections natively from raw lists
                    // me.loadTenantOptions(data);
                }
            },
                    ...
                    click: (me, btn) => {
                        ...
                        vsapi
                            .call(
                                [
                                    main_view.base_url,
                                    "/prm/tenant/request-service/save",
                                ].join(""),
                                data,
                                btn,
                                null,

...
const RoleTabView = new function(){
    ...
    this.displayRolePermissions = function(role_id){
        ...
        vsapi.call(`${main_view.base_url}/api/permissions`, p).then(res => {
            if(res.status_code === 200){
                let rows = StringSanitizer.sanitizeObject(res.data);
                let i = 0, c;
...
//         let role_name = RoleManagementComponent.selected_role_name;
//         if(!role_name) role_name = "This role";
//         RoleManagementComponent.self[0].querySelector('#_um_roletab_module_text').textContent = `${role_name} can use these modules`;

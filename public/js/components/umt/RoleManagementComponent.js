...
//  console.log(mThis.tblCard_body);
    ...
    this.loadRoles = (filter, onFinish)=>{
        ...
        vsapi.call1(`${main_view.base_url}/api/role/list`,filter,null,false).then(res =>{
...
/** begin:: AppPanel definition */ 
 this.AppPanel = new function(){
     ...
     this.loadApps =()=>{
        ...
        vsapi.call1(`${main_view.base_url}/api/role/apps`,p,false,false).then(res=>{
     ...
     this.toggleCheck = (btn)=>{
       ...
       function onAppStatusChange(btn,app_id, allowed ) {
           ...
           vsapi.call1(`${main_view.base_url}/api/role/apps/set-status`,p,false,false,false).then(res =>{
...
/** begin: UserPanel defintion */
 this.UserPanel = new function(){
    ...
    //                   return;
    //                }
    //                vsapi.call1(`${main_view.base_url}/api/user/set-password`,p,btn,false,false).then(res=>{
    //                     if (res.status_code ==200){
    //                         me.hide();
    ...
    this.deleteUser = (user_id)=>{
      ...
      cv_interact.confirm('Delete this user permanently?',{context:"delete",title:"Delete User"}, e=>{
         if(e){
             vsapi.call1(`${main_view.base_url}/api/user/delete`,p,false,false).then(res =>{
                 if(res.status_code ==200){
    ...
    this.btnAddRoleMember.onclick = e =>{
        ...
        let op = {
            ...
            "onClose":(users) =>{
               ...
               vsapi.call1([main_view.base_url, '/api/role/add-members'].join(''),p,null,false).then(res =>{
                  if(res.status_code == 200){
                     let d = res.data;
    ...
    this.displayModules = (role_id,app_id)=>{ 
        ...
        mThis.modulesList = mThis.modulesList || new  UMExpandItemView(mThis.div_modules,{
            ...
            onStatusChange:(statusInfo,item_id,parent_id,checkBox)=>{
                ...
                vsapi.call1(`${main_view.base_url}/api/role/modules/set-status`,p,false,false).then(res =>{
                    if(res.status_code ==200){
                       return;
                    }else{
        ...
        vsapi.call1(`${main_view.base_url}/api/role/modules`,p,false,false,false).then(res =>{
            console.log(3,res);
    ...
    this.displayPermissionList = (role_id, app_id,search_value)=>{
        ...
        mThis.permissionList = mThis.permissionList || new UMExpandItemView( mThis.div_permissions,{
            ...
            onStatusChange:(statusInfo,item_id,parent_id, checkBox)=>{
                ...
                vsapi.call1(`${main_view.base_url}/api/role/permissions/set-status`,p,false,false).then(res =>{
                     if(res.status_code ==200){
                        return;
                     }else cv_interact.warning(res.error_message);
                });
            ...
            vsapi.call1(`${main_view.base_url}/api/role/permissions`,p,false,false,false).then(res =>{
                let data = res.status_code ==200 ? res.data : [];
    ...
    this.displayReportList = async (role_id,app_id,search_value) => {
        ...
        mThis.reportList = mThis.reportList || new  UMExpandItemView(mThis.div_reports,{
            ...
            onStatusChange:(statusInfo,item_id,parent_id)=>{
                ...
                vsapi.call1([main_view.base_url,'/api/role/reports/set-status'].join(''),p, null,false).then(res =>{
                    if (res.status_code==200){
                    }else cv_interact.warning(res.error_message);
                });
            }
        ...
        vsapi.call1([main_view.base_url, '/api/role/reports'].join(''), p,false,false).then(res =>{
...
/**end: ReportPanel definition */
 ...
 async function getAccessibleApps(){
    ...
    const res = await vsapi.call1(`${main_view.base_url}/api/role/accessible-apps`,p,false,false,false);
               ...
               if (lnk){
                 cv_interact.confirm(['Are you sure to remove the selected user from ',mThis.selected_role.name || 'the role', '?'].join(''),{"title":"Remove User","context":'delete', "confirmButtonText":"Remove"},e =>{
                     if(e){
                         ...
                         vsapi.call1([main_view.base_url, '/api/role/remove-member'].join(''),p,null,false).then(res =>{
                             if(res.status_code ==200){
                                 mThis.userListView.showPage(RoleTabView.UserPanel.getFilterData());
                             }else cv_interact.warning(res.error_message);
                         });
             ...
             if(lnk){
                if(!AuthManager.allowed(113)) return;
                    ...
                    cv_interact.confirm(['Do you want to ',action.toLowerCase() ,' user ',user_name.replace(/^\w/, (c) => c.toUpperCase()),'?'].join(''),{
                        ...
                        {
                            delete(op.user_name);
                            vsapi.call1(`${main_view.base_url}/api/user/status/update`,op,lnk,false).then(res => {
                                if(res.status_code === 200)
                                {
...
const RoleDialog = (()=>{
          ...
          buttons:[
            ...
            },
            {
               ...
               click:(modal,btn,divModal)=>{
                   ...
                   vsapi.call1(`${main_view.base_url}/api/role/save`, p,null,false).then(res => {
                       if(res.status_code == 200){
                           const d = res.data;
                           modal.hide();
  ...
  const PrintDialog = new function(){
        ...
        if(d)
        {
            let html = `<div class="d-block position-relative">
                            <div class="height-logo-report position-absolute float-start">
                                <img style="max-width: 100px; max-height: 100px;" class=" object-fit-scale set-min-size-logo" src="${main_view.base_url}/assets/images/logo/kwis-logo.png" alt=""/> alt="logo"/>
                            </div>
    ...
    this.getCompanyLogo_url = ()=>{
         return vsapi.call1(`${main_view.base_url}/api/company/details`,null,null,false).then(res => {
			if(res.status_code ==200){
    ...
    this.prepareUserPrint = (div, d , users) => {
        ...
        if(users)
        {
            let html = `<div class="d-block position-relative">
                            <div class="height-logo-report position-absolute float-start">
                                <img style="max-width: 100px; max-height: 100px;" class="CompanyLogo object-fit-scale set-min-size-logo" src="${main_view.base_url}/assets/images/logo/kwis-logo.png" alt="logo"/>
                            </div>
                            <div class="d-flex flex-column gap-2 justify-content-center align-items-center set-min-size-container-title">
    ...
    this.permissionUserPrint = (div, d , users) => {
        ...
        if(users)
        {
            let html = `<div class="d-block position-relative">
                            <div class="height-logo-report position-absolute float-start">
                                <img style="max-width: 100px; max-height: 100px;" class="CompanyLogo object-fit-scale set-min-size-logo" src="${main_view.base_url}/assets/images/logo/kwis-logo.png" alt=""/>
                            </div>
                            <div class="d-flex flex-column gap-2 justify-content-center align-items-center set-min-size-container-title">
    ...
    this.loadFormDetails = (div, op, onFinish = null) => {
        ...
        if (op.action === 'gen_role')
        {
            ...
            vsapi.call1(`${main_view.base_url}/api/role/listForPrint`, {
        ...
        // console.log(3333,op);
            ...
            // api/role/members
            vsapi.call1(`${main_view.base_url}/api/role/members`, {
                role_id: op.role_id
            }, null).then(res => {
                if(res.status_code === 200)
                {
                    ...
                    // user.map( u =>{
                        ...
                        // vsapi.call1(`${main_view.base_url}/api/user/authorization-report`, {
                        //     role_id: op.role_id,
                        //     user_id: u.id
                        // }, null).then(res => {
                        //     if(res.status_code === 200)
        ...
        // console.log(3333,op);
            ...
            vsapi.call1(`${main_view.base_url}/api/role/modules`, {
                role_id: op.role_id
            }, null).then(res => {
                if(res.status_code === 200)
                {
        ...
        // console.log(3333,op);
            ...
            vsapi.call1(`${main_view.base_url}/api/role/permissions`, {
                role_id: op.role_id
            }, null).then(res => {
                if(res.status_code === 200)
                {
            ...
            vsapi.call1(`${main_view.base_url}/api/role/members`, {
                ...
                if(res.status_code === 200)
                {
                    ...
                    // user.map( u =>{
                        ...
                        vsapi.call1(`${main_view.base_url}/api/user/authization/report`, {
                            role_id: op.role_id,
                            user_id: u[0].id
                        }, null).then(res => {
                            if(res.status_code === 200)

...
const RoleTabView = new function(){
    ...
    //this.official_code = RoleManagementComponent.;
    ...
    const user_cols = [
        {
         ...
         {
            title:`ID`,
            //className: `${mThis.official_code? 'id':'d-none'}`,
            data:(data,index,tr)=>{
                return data.official_code ?? 'N/A';
            }
        ...
        },
        {
           title:"Created By",
           data:(data,index,tr)=>{
               const creator = data.create_user ?? data.update_user;
               const createDate = data.create_date ?? data.update_date;
               return ['<span class="d-block">',creator ,'</span><span class="d-block text-muted"><small>', createDate,'</small></span>'].join('');
           }
        ...
        },
        {
            ...
            data:(data,index,tr)=>{
                ...
                `<a data-id="`,data.id,`" data-loginname="${data.login_name}" href="javascript:void(0)" class="lnk-reset-password"><i class="fa fa-key text-warning"></i></a>`,
                `<a data-id="`,data.id,`"  data-loginname="${data.login_name}" href="javascript:void(0)" data-action="`,(data.is_locked ==1? 'unlock':'lock'),`" class="lnk-lock-user"><i class="${data.is_locked==1? 'fa fa-unlock text-info':'fa fa-lock text-danger'}"></i></a>`,
                `<a data-id="`,data.id,`"   data-loginname="${data.login_name}" href="javascript:void(0)" class="lnk-remove-user"><span class="p-1 rounded-4"><i class="fa fa-times text-danger fw-bold"></i></span></a>`,
                `<a data-id="`,data.id,`"  data-loginname="${data.login_name}"  href="javascript:void(0)" class="user_action"><span class="p-1 rounded-4"><i class="fa fa-tasks text-info fw-bold"></i></span></a>`,
                `</div>`].join('');
            }
  ...
  const PrintDialog = new function(){
    ...
    this.prepareUserPrint = (div, d) => {
        ...
        if(d)
        {
            let html = `<div class="d-block position-relative">
                            <div class="height-logo-report position-absolute float-start">
                                <img style="max-width: 100px; max-height: 100px;" class="CompanyLogo object-fit-scale set-min-size-logo" src="${company_info.logo_url || "" }" alt=""/>
                            </div>
                            <div class="d-flex flex-column gap-2 justify-content-center align-items-center set-min-size-container-title">
                                <h4 class="text-center text-uppercase">Users List By Role : ${users[0].role_name}</h4>
                                <p class="text-center w-100 fs-5-1 get-subtitle"> </p>
                            </div>
                        </div>`;
            const tHead = `
    ...
    this.preparePermissionPrint = (div, d) => {
        ...
        if(data)
        {
            let html = `<div class="d-block position-relative">
                            ...
                            <div class="d-flex flex-column gap-2 justify-content-center align-items-center set-min-size-container-title">
                                <h4 class="text-center text-uppercase">Permission List By Role : </h4>
                                <p class="text-center w-100 fs-5-1 get-subtitle"> </p>
                            </div>
                        </div>
                        <style>
    ...
    this.loadFormDetails = (div, op, onFinish = null) => {
        ...
        if (op.action === 'gen_role')
        {
            ...
            vsapi.call(`${main_view.base_url}/api/role/list/print`, {
                ...
                {
                    //console.error('Failed to load receipt details at api/role/list/print');
    ...
    this.show = (options) => {
        ...
        options.action === 'gen_user' ? (title = `Preveiw Users Print`, btn_name = 'Print Now') : options.action === 'gen_role' ? (title = `Preveiw Role Print`, btn_name = 'Print Now') : (title = `Preveiw Dialog`, btn_name = 'Print Now');

...
var CountryZonesComponent  = new function () {
  ...
  this.cols = [
   ...
   
    {
      title:"Create By",
      data:(data,index,tr)=>{
        return ['<div class="d-flex" ><i class="fas mt-2 text-success fa-user"></i><span class="d-block p-1">',(data.create_user ?? 'គ្មាន'),
  ...
  this.init = function () {
    ...
    mThis.listView = new ListView('_sttn_delivery_zones', {
      fetchApi: `${main_view.base_url}/dms/location/countries`,
    ...
    this.sh_container = mThis.listView.getListContainer();
    ...
    mThis.btnPrint.on('click', (e) => {
      ...
      vsapi.call(`${main_view.base_url}/dms/location/countries`, p, null).then(res => {
...
const ZoneDialog1 = new function () {
  ...
  this.btnOK.on('click', function (e) {
    ...
    vsapi.call(`${mThis.base_url}/abm/country/save`, p, null).then(res => {
      if (res.status_code === 200) {
  ...
  this.prepareFormData = (id, def, onFinish) => {
    ...
    //       }
    // });
    vsapi.call(`${main_view.base_url}/abm/country/details`, { 'id': id }, null).then(res => {

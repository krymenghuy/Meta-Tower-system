...
var CompanyComponent = (function() {
    ...
    mThis.displayCompanyInfo = () => {
        vsapi
            .call(
                `${mThis.base_url}/api/company/details`,
                false,
                null,
    ...
    //             if (com.logo_url.status_code !== 200) {
    //                 mThis.hideLogo();
    //             } else {
    //                 el.src = logoUrl;
    //                 mThis.showLogo();
        ...
        mThis.btnChooseLogo.addEventListener("click", function(e) {
            ...
            FileChooser.chooseFile(null, d => {
                if (d) {
                    ...
                    vsapi
                        .call(
                            `${mThis.base_url}/api/company/save-logo`,
                            p,
                            null,
                            false
                        )
                        .then(res => {
                            if (res.status_code === 200) {
                                ...
                                cv_interact.warning(
                                    res.error_message || "Failed to save logo"
                                );
                            }
                        });
                }
            ...
            cv_interact.confirm(
                ...
                confirmed => {
                    if (confirmed) {
                        vsapi
                            .call(
                                `${mThis.base_url}/api/company/delete-logo`,
                                null
                            )
                            .then(res => {
                                if (res.status_code === 200) {

          ...
          function renderGroupTable($rows,$date,&$grand_total_driver,&$grand_total_taxi,&$grand_total_fees,&$grand_total_merchant){   
             $start_table = '<div class="w-100 bg-primary text-center p-2"><span class="p-2 text-white fw-semibold">'.date('d M Y',strtotime($date)).'</span></div><table class="table table-bordered table-hover">
                    ...
                    foreach($rows as $row){
                        ...
                        if(in_array($row->status_id,[5,6]))  $delivery_time='មិនទាន់';
                        ...
                        $remarks = $row->status_id ==8? '<div class="d-flex flex-column"><span class="d-block text-success">Success</span><span style="font-size:0.8em">'.$delivery_time.'</span><span class="d-block text-secondary">'.$admin_notes.'</span></div>':$row->failure_notes;
                        ...
                        '<td><span class="d-block">'.$cur.number_format($fees,2,'.','').'</span><span class="d-block p-1 text-right text-warning"><small>'.$row->df_payer.'</small></span></td>'.
                    ...
                    <div class="d-block">
                       <div class="d-flex align-items-center">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <p>
                                <?php echo number_format($data->exchangeRateInfo->rate,2); ?>
                            </p>
                        </div>
                        <div class="d-flex align-items-center">
                            <p class="text-primary fw-semibold"><small>Item listing is based on finish date</small></p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <table class="table">
                  ...
                  <tbody>
                    <?php
                      ...
                      $item = (object)$data->summary_info['remaining'];
                      echo '<tr><td>'.$item->label.'</td> <td>'.$item->count.'</td><td></td><td>'.$cur.number_format($item->amount,2,'.','').'</td></tr>';

                      $item = (object)$data->summary_info['new_arrival'];
                      echo '<tr><td>'.$item->label.'</td> <td>'.$item->count.'</td><td></td><td>'.$cur.number_format($item->amount,2,'.','').'</td></tr>';

                      $item = (object)$data->summary_info['total_count'];
                      echo '<tr class="total-row"><td>'.$item->label.'</td> <td>'.$item->count.'</td><td></td><td>'.$cur.number_format($item->amount,2,'.','').'</td></tr>';

                      $item = (object)$data->summary_info['delivered'];
                      echo '<tr><td>'.$item->label.'</td> <td>'.$item->count.'</td><td></td><td>'.$cur.number_format($item->amount,2,'.','').'</td></tr>';

                      $item = (object)$data->summary_info['returned'];
                      echo '<tr><td>'.$item->label.'</td> <td>'.$item->count.'</td><td></td><td>'.$cur.number_format($item->amount,2,'.','').'</td></tr>';

                      $item = (object)$data->summary_info['failed'];
                      echo '<tr><td>'.$item->label.'</td> <td>'.$item->count.'</td><td></td><td>'.$cur.number_format($item->amount,2,'.','').'</td></tr>';

                      $item = (object)$data->summary_info['at_warehouse'];
                      echo '<tr><td>'.$item->label.'</td> <td>'.$item->count.'</td><td></td><td>'.$cur.number_format($item->amount,2,'.','').'</td></tr>';

                      $item = (object)$data->summary_info['on_delivery'];
                      echo '<tr><td>'.$item->label.'</td> <td>'.$item->count.'</td><td></td><td>'.$cur.number_format($item->amount,2,'.','').'</td></tr>';
                    ?>  
                  </tbody>
                </table>

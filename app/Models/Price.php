...
class Price extends Model
{
    ...
    function saveBaseFee($d){
        ...
        $delivery_type = isset($d->delivery_type)?sanitize($d->delivery_type):'all';
        $zone_code = isset($d->zone_code)?sanitize($d->zone_code):'all';
        $id =isset($d->id)?sanitize($d->id):0;
        ...
        $never_expires = isset($d->never_expires)?sanitize($d->never_expires):1;
    ...
    function setMerchantPriceList($d){
        ...
        $list_exists = DB::table('price_list_names')->where('id',$price_list_id)->exists();
        if(!$list_exists){
    ...
    function saveCODFeeCharge($d){
        ...
        $branch_id = sanitize($ss->branch_id);
        $sender_id = isset($d->sender_id)?sanitize($d->sender_id):null;
        $cod_fee_percent = isset($d->cod_fee_percent)?sanitize($d->cod_fee_percent):0;
        $start_date = isset($d->start_date)? convertDate($d->start_date):null;
        $end_date = isset($d->end_date)? convertDate($d->end_date):null;
        $never_expires =isset( $d->never_expires)? sanitize($d->never_expires):1;

        $cod_fee = isset($d->cod_fee)?sanitize($d->cod_fee):0;
        $charge_option = isset($d->charge_option)?strtolower(sanitize($d->charge_option)):null;
        if (!(bool)strtotime($start_date)) $start_date = date('Y-m-d');
    ...
    function savePriceLine($d){
        ...
        $sender_id = isset($d->sender_id)?sanitize($d->sender_id):null; // if not @sender_id specified => it must be NULL here, not zero
        $delivery_type = isset($d->delivery_type)?sanitize($d->delivery_type):'Normal';
        //$zone_code = isset($d->zone_code)?sanitize($d->zone_code):'all'; //if not zone_code specified => the price is applied to all zones
        $zone_codes = isset($d->zone_codes)?sanitize($d->zone_codes):null;
        $sender_ids = isset($d->sender_ids)?sanitize($d->sender_ids):'all';
        $base_fee = isset($d->base_fee)?sanitize($d->base_fee):0;
        $price_option = isset($d->price_option)?sanitize($d->price_option):null;
        $delivery_fee = isset($d->delivery_fee)?sanitize($d->delivery_fee):0;
        $price_per_kg = isset($d->price_per_kg)?sanitize($d->price_per_kg):0;
        $never_exires =isset($d->never_expires)?sanitize($d->never_expires):1;
        $start_kg = isset($d->start_kg)? sanitize($d->start_kg):0;
        $end_kg =isset($d->end_kg)?sanitize($d->end_kg):0;
    ...
    function getApplicableZones($d){
        ...
        $zone_codes = sanitize($zone_codes);
    ...
    function getApplicableSenders($d){
        ...
        $sender_ids = sanitize($sender_ids);
   ...
   function saveCODFeeBySender($d){
        ...
        if (!prn_allowed(2)) return '@'; //need permission to do this task
        $branch_id = $ss->branch_id;
        $id = isset($d->id)?sanitize($id->id):null;
        $cod_fee_percent = isset($d->cod_fee_percent)?sanitize($d->cod_fee_percent):0;
        $remarks = isset($d->remarks)?sanitize($d->remarks):null;
        ...
        $never_exires = isset($d->necer_expires)?sanitize($d->never_expires):1; //default to 1
        $sender_id = isset($d->sender_id)?sanitize($d->sender_id):null;
    ...
    function savePriceLineInfo($d){
        ...
        $zone_codes = isset($d->zone_codes)?$d->zone_codes:null;
        $delivery_type = isset($d->delivery_type)?$d->delivery_type:null;
        $section = isset($d->section)?$d->section:null; /** @section = {'above','below'} **/

        $base_fee = isset($d->base_fee)?$d->base_fee:0;
        $price = isset($d->delivery_fee)?$d->delivery_fee:0;
        if($price <=0) $price = isset($d->price)?$d->price:0;

        $price_option = isset($d->price_option)?$d->price_option:'fixed'; //default to "fixed"

        $result = (object)['id'=>null,'error_message'=>null,'status'=>'OK'];

        //validate data inputs
        if ($section !='below' && $section !='above') {
            $result->error_message ='@section is not valid';
            $result->status='Error';
            return $result;
        }

         if (empty($price_list_id)) {
             $result->error_message ='Price list ID is not valid';
             $result->status='Error';
             return $result;
         }
         if (empty($zone_codes)) {
            $result->error_message ='The provided zone codes are not correct';
            $result->status='Error';
            return $result;
        }

        $dtypes =['normal','fast'];
        if (!in_array(strtolower($delivery_type),$dtypes)) {
        ...
        foreach($rows as $row) {
          DB::table('price_list')->where('branch_id',$branch_id)->where('zone_codes',$zone_codes)->where('delivery_type',$delivery_type)->where('section',$section)->where('price_list_id',$price_list_id)->update(array(
              'base_price'=>$base_fee,
    ...
    function createPriceList($d){
        ...
        $exists = DB::table('price_list_names')->where('branch_id',$branch_id)->where('name',$name)->limit(1)->exists();
        if($exists) {
        ...
        DB::table('price_list_names')->insert(array(
            'branch_id'=>$branch_id,
            'name'=>$name,
            'kg_marker'=>$kg_marker,
            'create_user'=>$ss->login_name,
    ...
    function deletePriceList($d){
        ...
        DB::table('price_list_names')->where('branch_id',$branch_id)->where('id',$id)->delete();
    ...
    function getComboItems_price_list($d){
        ...
        if (!prn_allowed(2)) return '@'; //need permission to do this task
        $branch_id = $ss->branch_id;
        $rows = DB::table('price_list_names AS ps')->where('ps.branch_id',$branch_id)->selectRaw("id,name,kg_marker")->get();
        return $rows;
    }

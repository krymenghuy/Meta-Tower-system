...
class Package //extends Model
{
    ...
    function getPriceListIdBySender($sender_id){
      ...
      return DB::table('sender AS s')->where('id',$sender_id)->selectRaw($sender_id.' AS sender_id,price_list_id,IFNULL(cod,0) AS cod,IFNULL(cod_fee,0) AS cod_fee')->take(1)->first();
    } 
 
    //getDeliveryPrice|getDeliveryPriceInfo|getFee|getServiceFee 
    //returns object {'base_fee','delivery_fee','cod_fee_percent'}
    function getDeliveryPriceInfo($branch_id,$sender_id,$delivery_type,$zone_code='all',$billed_kg=0,$cod=null) {
      //$branch_id = $ss->branch_id;
      $delivery_fee =-1;
      $base_fee =-1;
      ...
      $data = (object) ['delivery_fee'=>-1,'base_fee'=>-1,'cod_fee_percent'=>-1,'error_message'=>'Failed to find matched price','status'=>'Error','status_code'=>405];
      $price_list_id =null;
      ...
      if(!in_array(strtolower($delivery_type),$dTypes)) return DV::error('Delivery type is not correct!');
      ...
      $more_wheres = "1=1 ".$str_kg.$str_price_list.$str_zone;
      $row = DB::table($table)->where('l.branch_id',$branch_id)->where('l.delivery_type',$delivery_type)->whereRaw($more_wheres)->selectRaw("IFNULL(l.base_price,0) AS base_fee,IFNULL(l.price,0) AS price,IFNULL(l.price_per_kg,0) AS price_per_kg,IFNULL(l.start_kg,0) AS start_kg, l.price_option")->take(1)->first();
      ...
      $data = (object)['status'=>'OK','error_message'=>null,'delivery_fee'=>number_format($delivery_fee,2),'base_fee'=>$base_fee,'cod_fee_percent'=>$cod_fee_percent,'cod'=>$cod];
    ...
    function getSenderInfoByCode($ss,$sender_code) {
     ...
     public function getSenderInfoByOrderCode($data) {
      ...
      function getFilterOptions($ss=null) {
          ...
          $data->senders  = DB::table('sender AS s')->selectRaw('s.id, CONCAT(s.code,\' | \', s.`name`) AS sender_name')->where('branch_id',$branch_id)->orderByRaw('name ASC')->get();
...
static function list($arr,$ss=null) {   
  ...
  if(!$fresh){
    ...
    foreach($overdue_queries as $q){
      ...
      $str_time = 'TIMESTAMPDIFF(HOUR, p.arrival_time, NOW()) >='.$hours_ago;
 ...
 static function list_overdue($arr, $status_id, $ss=null) { 
  ...
  $wheres = [
    ...
    '6'=>'TIMESTAMPDIFF(HOUR, p.arrival_time, NOW()) >='.self::$overdue_alert_times['6'].' AND p.status_id ='.$status_id,
    '9'=>'TIMESTAMPDIFF(HOUR, p.arrival_time, NOW()) >='.self::$overdue_alert_times['9'].' AND p.status_id ='.$status_id
  ]; 

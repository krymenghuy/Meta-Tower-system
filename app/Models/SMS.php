...
class SMS //extends Model
{
    ...
    static function getMessageTemplate($ss,$purpose,$otp=null){
        $purpose = strtolower($purpose);
        if(!$ss){
            $subs= getCurrentSubs(true);
            if ($subs) $ss = (object)['subs_id'=>$subs->id,'subscriber_id'=>$subs->subscriber_id];
        }
        $customer_id = $ss? $ss->subscriber_id : null;
        $b = null;
        try{
            if ($customer_id){
                try{
                      if (!isBinary($customer_id)) $customer_id = hex2bin($customer_id);
                      $b = DB::table('um_branches as b')->where('id',$customer_id)->selectRaw('name,phone_number')->first();
                }catch( \Exception $e){
                    
                }
            }
        }catch (\Exception $e){
            \Log::info('Failed to get company name for sending SMS. Subscriber_id : '.is_string($customer_id) ? $customer_id: ' (empty)');
            \Log::error($e->getMessage());
            \Log::error($e->getTraceAsString());
         }

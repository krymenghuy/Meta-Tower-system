...
class PendingTask //extends Model
{
    ...
    static function create($action_name,$subs_id,$branch_id,$user_id,$org_value,$new_value,$otp_code){
         ...
         if(!User::existsBy('id',$user_id)) return DV::error('Failed to create pending update task because the user identity is not valid');  
       ...
       $user = User::getProps($user_id,'login_name');
       $login_name = $user->login_name;
       DB::table('change_info_otp')->where('subs_id',$bin_subs_id)->where('user_id',$user_id)->where('action_name',$action_name)->delete();
       
       $nowTime = getNowTime();
       DB::table('change_info_otp')->insert([
        'subs_id'=>$bin_subs_id,
        'branch_id'=>$branch_id,
    ...
    static function finish($action_name,$user_id,$otp_code){
        $row = DB::table('change_info_otp')->where('user_id',$user_id)->where('action_name',$action_name)->selectRaw("login_name,org_value,new_value,otp_code")->first();
        //$org_value = null;
        $new_value = null;
        $login_name = null;
        $org_otp_code = null;
        ...
        $task_done = false;
        $user = User::getProps($user_id,'id,user_class,official_id,phone_number');
                ...
                //if(!$id) return DV::error('Merchant identity is unexpectedly invalid!');
                 switch($action_name){
                     case 'change_phone_number':{
                        if(!$new_value) return DV::error('មិនទាន់អាចប្ត្តរលខទូរសព្ទ័ because the new phone number was empty');
                        DB::table('sender')->where('id',$user->official_id)->update(['phone_number'=>$new_value]);
                        $inputs = ['phone_number'=>$new_value];
                        if (UMTSettings::$user_classes['merchant']['login_type'] ==='phone'){
                             $inputs['login_name'] = $new_value;
                        }
                        User::updateProps($user->id,$inputs);
                        $task_done =true;
                        break;
                     }case 'change_email':{
                        //DB::table('sender')->where('id',$user->official_id)->update(array('email'=>$new_value));
                         //\App\Models\Umt\User::updateEmail($new_value,$user_id);

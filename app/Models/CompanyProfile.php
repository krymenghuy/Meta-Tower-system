...
class CompanyProfile //extends Model
{
  ...
  static function connectWithUs($ss){
   ...
   function getDetails($ss) {
      ...
      foreach($rows as $row) {
        $row->logo_url =self::logoUrl($branch_id);
        return $row;
      }
      return null;
   } 
  ...
  static function details($branch_id) {
     $row = DB::table('um_branches')->where('branch_id',$branch_id)->selectRaw("branch_id,name,name_kh,`address`,address_kh,phone_number,email,first_cp_name,second_cp_name,first_cp_phone,second_cp_phone")->take(1)->first();
     if(!$row) return null;
      $row->logo_url =self::logoUrl($branch_id);
      return $row;
  ...
  static function logoUrl($branch_id){
    // if(intval($ss) !==false) $branch_id  = $ss;
    // else $branch_id = $ss->branch_id;
    $branch = DB::table('um_branches')->where('branch_id',$branch_id)->selectRaw('logo_file_name')->first();
  ...
  {
    $ss = $ss?$ss:$this->getUserInfo();
    return self::logoUrl($ss->branch_id);
  }

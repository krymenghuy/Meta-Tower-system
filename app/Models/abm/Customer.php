...
return null;
  ...
  function save($arr,$id=null,$ss=null){
    ...
    if(!$d->phone_number) return DV::error('Phone number is required for valid Customer account');
  ...
  if ($search_value) 
  {
   ...
   foreach($rows as $row){
    ...
    // $row->bank_accounts = self::bankAccounts($row->id,null);
     ...
     if($row->photo_file_name) $row->image_url = PublicStorage::getUrl($row->branch_id,'customer','image').$row->photo_file_name;
...
static function details($id,$ss,$includeProfilePicture=false){
  ...
  if (!$row) return null;
    ...
    static function getFormOptions($id,$ss){   
        ...
        if($id>0) $customer_details = self::details($id,$ss);
        $data= (object)[];

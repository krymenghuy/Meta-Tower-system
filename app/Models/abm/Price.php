...
class Price //extends Model
{
    ...
    static function price_list_name_exists($name,$id=null){
        ...
        $rows = DB::table('os_customer_price_list_names')->where('name',$name)->whereRaw($str_id)->select("id")->take(1)->get();
        return isset($rows[0]);
    }
    ...
    function renamePriceList($new_name,$id=null,$ss=null){
       ...
       DB::table('os_customer_price_list_names')->where('id',$id)->where('branch_id',$branch_id)->update(['name'=>$new_name]);
       return DV::success();
    }
    ...
    function setMerchantPriceList($d){
        ...
        $list_exists = DB::table('os_customer_price_list_names')->where('id',$price_list_id)->exists();
        if(!$list_exists){
    ...
    function savePriceLine($d){
        ...
        $table ='os_customer_price_list_details';
   ...
   function saveCODFeeBySender($d){
    ...
    function getPriceList_data($d){
        ...
        $rows =[];
        $kg_marker =0;
        $rows = DB::table('os_customer_price_list_names')->where('branch_id',$branch_id)->where('id',$price_list_id)->selectRaw("id,kg_marker")->get();
        ...
        /** $rows query conditions are, for example => assuming x = price_list('A').kg_marker, then (start_kg =x OR end_kg =x) **/
          $rows = DB::table('os_customer_price_list_details AS l')
    ...
    function savePriceLineInfo($arr=[],$id=null,$ss=null){
        ...
        // $rows = DB::table('os_customer_price_list_names AS n')->where('n.branch_id',$branch_id)->where('id',$price_list_id)->selectRaw('kg_marker')->limit(1)->get();
        // foreach($rows as $row) $kg_marker = $row->kg_marker;
        ...
        $rows = DB::table('os_customer_price_list_details AS l')->where('branch_id',$branch_id)->where('zone_code',$zone_codes)->where('item_type',$item_type)->selectRaw("l.id")->get();
        //if the pricing condition already exist => then UPDATE (base_fee, delivery_fee, price_option) of the existing one
        foreach($rows as $row) {
          $qres = DB::table('os_customer_price_list_details')->where('branch_id',$branch_id)->where('zone_code',$zone_codes)->where('item_type',$item_type)->where('price_list_id',$price_list_id)
          ->update([
            //   'base_price'=>$base_fee,
        ...
        DB::table('os_customer_price_list_details')->insert([
            'branch_id'=>$branch_id,
            'price_list_id'=>$price_list_id,
            'item_type'=>$item_type,
    ...
    function updateZoneCodes($d){
        ...
        $res = DB::table('os_customer_price_list_details')->where('branch_id',$branch_id)->where('zone_code',$org_zone_codes)->where('country_id',$country_id)->where('item_type','doc')->where('price_list_id',$price_list_id)->update(array(
            'zone_code'=>$zone_codes,
        ...
        $res = DB::table('os_customer_price_list_details')->where('branch_id',$branch_id)->where('zone_code',$org_zone_codes)->where('item_type','non_doc')->where('country_id',$country_id)->where('price_list_id',$price_list_id)->update(array(
            'zone_code'=>$zone_codes,
            'country_id'=>$country_id,
    ...
    function savePriceLineZones($d) {
        ...
        // $kg_marker =-1;
        // $rows = DB::table('os_customer_price_list_names AS l')->where('branch_id',$branch_id)->where('id',$price_list_id)->selectRaw("l.id,l.kg_marker")->limit(1)->get();
        // foreach($rows as $row){
        ...
        // $end_kg = $kg_marker;
        // $start_kg = -1;
        DB::table('os_customer_price_list_details')->insert(array(
            'branch_id'=>$branch_id,
            'price_list_id'=>$price_list_id,
            'zone_code'=>$zone_codes,
            'country_id'=>$country_id,
    ...
    function createPriceList($d){
        ...
        $exists = DB::table('os_customer_price_list_names')->where('branch_id',$branch_id)->where('name',$name)->limit(1)->exists();
        if($exists) {
        ...
        DB::table('os_customer_price_list_names')->insert(array(
            'branch_id'=>$branch_id,
            'name'=>$name,
            'kg_marker'=>$kg_marker,
            'create_user'=>$ss->login_name,
    ...
    function deletePriceList($d){
        ...
        DB::table('os_customer_price_list_names')->where('branch_id',$branch_id)->where('id',$id)->delete();
    ...
    function getComboItems_price_list($d){
        ...
        //need permission to do this task
        $branch_id = $ss->branch_id;
        $rows = DB::table('os_customer_price_list_names AS ps')->where('ps.branch_id',$branch_id)->selectRaw("id,name,kg_marker")->get();
        return $rows;
    }

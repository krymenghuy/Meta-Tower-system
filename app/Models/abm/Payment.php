...
{   
    ...
    function getSuplierList(){
        ...
        return DB::table('os_payments')->selectRaw('id,name, phone_number, email, address,status_code,price_list_id,formatDate(create_date) as create_date,DATE_FORMAT(create_date,\'%r\') AS request_time')->get();
    }
    ...
    function checkUniquePerson($branch_id,$phone_number,$id=null){
        ...
        $x = DB::table('os_payments as s')->where('s.branch_id',$branch_id)->where("s.phone_number",$phone_number)->whereRaw($str_id)->select('id')->take(1)->exists();
    ...
    function getSuplierListPaginate($filter,$ss){
       ...
       // $query = DB::table('requirements as r')->whereRaw($str_srch)->selectRaw('r.id,r.description,r.status_id'.$projectName);
        $query = DB::table('os_payments as s')
    ...
    function setPriceList($price_list_id,$id=null,$ss =null){
        ...
        $p_name = $p->name;
        DB::table('os_payments')->where('id',$id)->update(array(
    ...
    function delete($id){
        ...
        $delete = DB::table('os_payments')->where('id',$id)->delete();
    ...
    function updateStatus($status_code,$id=null,$ss=null){
        ...
        $x = DB::table('os_payments')->where('id',$id)->update([
    ...
    static function details($id,$ss,$includeProfilePicture=false,$includeBankAccount=true){
        $branch_id = $ss->branch_id;    
        $row = DB::table('os_payments')->selectRaw('id,name,code, phone_number, email,sales_agent_id,photo_file_name, address,status_code,price_list_id,formatDate(create_date) as create_date,DATE_FORMAT(create_date,\'%r\') AS request_time')->where('branch_id',$branch_id)->where('id',$id)->take(1)->first();
        if (!$row) return null;
     ...
     function saveProfilePicture($photo_data,$file_type = null,$id=null,$ss=null){
        $id = $id?$id:$this->id;
        $ss = $ss?$ss:$this->userInfo;
        $supplier = DB::table('os_payments')->where('id',$id)->selectRaw('id,branch_id,photo_file_name')->first();
        ...
        if($delete_image){
          ...
          DB::table('os_payments')->where('id',$id)->update(['photo_file_name'=>null]);
        }
        return PublicStorage::saveImage($ss->branch_id,self::$img_dir, null,$photo_data,null,['id'=>$id,'store'=>'os_payments.photo_file_name']);  
        
      }
      static function getProfilePicture($id)
  {
    $row = DB::table('os_payments ')->where('id', $id)->selectRaw('branch_id,photo_file_name')->first();
    if (!$row) {

...
class Supplier //extends Model
{
    ...
    function save($arr, $id = null, $ss = null)
    {
        ...
        $d = (object) $inputs;
        if(!$d->price_list_id)
            return DV::error('price list is required for supplier.');
        $photo = $d->photo;
        ...
        if ($id > 0) {
            ...
            if ($delete_prev_image) {
                ...
                if ($file_name)
                    PublicStorage::delete($branch_id, self::$img_dir, 'image', $file_name);
            ...
            if ($supplier_created) {
                $new_code = $this->getNextSenderCode($ss); // formatNumber($customer_id,5);
    ...
    // {
    //     // return JDV::result(DB::table('shipments')->selectRaw('zone_code,customer_id')->get());
    ...
    function checkUniquePerson($branch_id,$phone_number, $id = null)
    {
        ...
        if ($x)
            return 'Phone number "' . $phone_number . '" is already save...';
        return null;
    }
    ...
    static function list ($filter, $ss)
    {
        ...
        $query = DB::table('os_suppliers as s')
                ->join('os_supplier_price_list_names as p','p.id','=','s.price_list_id')
                ->whereRaw($str_srch)
        ...
        foreach ($rows as $row) {
            ...
            if ($row->photo_file_name)
                $row->image_url = PublicStorage::getUrl($row->branch_id, 'general', 'image') . $row->photo_file_name;
            unset($row->photo_file_name);
            if (!$row->image_url)
                $row->image_url = self::defaultImage($ss->branch_id);
        }
    ...
    {
        $row = DB::table('package as p')->join('sender as s', 's.id', '=', 'p.customer_id')->where('p.status_id', 8)->where('s.id', $id)->whereRaw('IFNULL(p.sender_pmt_status_id,0) =0')->selectRaw('COUNT(p.id) AS item_count,SUM(IFNULL(p.sender_total,0)) AS amount')->get()->first();
        if (!$row)
            return null;
        if ($row->item_count > 0)
    ...
    static function getFormOptions($id, $ss)
    {
        ...
        if ($id > 0)
            $supplier_details = self::details($id, $ss);
        $data = (object) [];

...
class Customer //extends Model
{
  ...
  function save($arr, $id = null, $ss = null)
  {
    ...
    $v_rule = [
      ...
      'primary_cp_percent' => '0|number|default=0',
      'secondary_cp_percent' => '0|number|default=0',
    ...
    $d = (object) $inputs;
    if($d->primary_cp_percent >100 || $d->secondary_cp_percent >100)
      return DV::error('Percent can not bigger than 100');
    if(!$d->price_list_id)
      return DV::error('price list is required for customer.');
    // return $inputs;
    $photo = $d->photo;
    unset($inputs['photo']);
    ...
    if ($id > 0) {
      if($delete_prev_image){
        $file_name = DB::table('sender as s')->where('s.id',$id)->value('s.photo_file_name');
        if($file_name) PublicStorage::delete(['subs_id'=>$subs_id,'dir'=>self::$img_dir],'image',$file_name);
        DB::table('sender as s')->where('s.id',$id)->update(['photo_file_name'=>null]);
      }
      PublicStorage::saveImage(['subs_id'=>$subs_id,'dir'=>self::$img_dir],null,$photo,null,['id'=>$id,'store'=>'sender.photo_file_name']);
  ...
  static function getProfilePicture($id)
  {
    ...
    $url = PublicStorage::getUrl(['subs_id'=>$subs_id, 'dir'=>'customer'], 'image') . $row->photo_file_name;
  ...
  static function list($arr, $ss)
  {
    ...
    $per_page = isset($d->per_page) ? $d->per_page : 8;
  ...
  static function list_all($arr, $ss)
  {
    ...
    foreach ($rows as $row) {
      ...
      if ($row->photo_file_name)
        $row->image_url = PublicStorage::getUrl($row->branch_id, self::$img_dir, 'image') . $row->photo_file_name;
      unset($row->photo_file_name);
      if(!$row->image_url) $row->image_url =self::defaultImage($ss->branch_id);
    }
    return $rows;
  ...
  static function getFormOptions($id, $ss)
  {
    ...
    $data->primary_cp = DB::table('os_affiliates as a')->join('os_contact_persons as cp','cp.affiliate_id','=','a.id')->where('cp.cp_type_id',1)->selectRaw('a.id, a.name as primary_cp_name')->get();
    $data->secondary_cp = DB::table('os_affiliates as a')->join('os_contact_persons as cp','cp.affiliate_id','=','a.id')->where('cp.cp_type_id',2)->selectRaw('a.id, a.name as secondary_cp_name')->get();

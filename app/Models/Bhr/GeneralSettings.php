...
class GeneralSettings //extends Model
{
   ...
   static function options_delivery_remark($category,$ss){
    ...
    static function options_employee($emp_status_id, $ss){
       $q = DB::table('employees as e')->where('e.subs_id',hex2bin($ss->subs_id))->selectRaw('id,name, sex, name_kh,phone_number,email,postion_id,photo_file_name');
       if($emp_status_id) $q->where('e.status_id',$emp_status_id);
       $rows = $q->get();
       foreach($rows as $row){
         $row->image_url = Employee::profilePicture($row->id);
         $position_title = DB::table('positions')->where('id',$row->postion_id)->value('title');

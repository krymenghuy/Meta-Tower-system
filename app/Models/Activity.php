...
class Activity //extends Model
{
    ...
    function requestChange($arr,$ss=null){
        ...
        $activity_info = null;
        if(!isset($arr['activity_info'])) return DV::error('Array Activity info missing');
        $activity_info = $arr['activity_info'];
        $success = 0;
        $cross_values = 0;
        foreach($activity_info as $info){
            ...
            $to_level_id = $inputs['level_id'];
            $to_session_id = $inputs['session_id'];
            $to_campus_id = $inputs['campus_id'];
            ...
            if($req_type_id == 1){ //* request level
               // if(!isset($to_level_id)) //return DV::error('level_id not found');
               ...
               if($req_exists){
                ...
                //if(!isset($to_campus_id)) //return DV::error('Campus not found or need to input');
                $req_exists = DB::table('requests')->where('request_type_id',$req_type_id)->where('student_id',$inputs['student_id'])->where('term_id',$studentInfo->term_id)->where('is_approve',0)->exists();
                if($req_exists){
                    $cross_values +=1;
                    continue;
                ...
                //if(!isset($to_session_id)) //return DV::error('Session not found or need to input');
                $req_exists = DB::table('requests')->where('request_type_id',$req_type_id)->where('student_id',$inputs['student_id'])->where('term_id',$studentInfo->term_id)->where('is_approve',0)->exists();
                if($req_exists){
                    $cross_values +=1;
                    continue;
    ...
    function approveGeneralListPaginateList($filter=[],$ss=null){
        ...
        $selectCols = 's.id as student_id,r.is_approve,r.id as request_id,s.file_name,s.name as student_name,s.code as student_code,s.name_kh,s.sex,s.date_of_birth,e.start_date as admission_date,e.session_id';

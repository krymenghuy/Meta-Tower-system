...
function makePayment($arr,$ss=null){
 ...
 function getSettledPackages_sender($trx_id,$ss=null){
    ...
    $rows = DB::select(DB::raw('SELECT p.id,p.qr_code as barcode,p.id as package_id,p.sender_pmt_status_id,p.delivery_type,p.sender_id,formatDate(p.arrival_time) AS arrival_date,formatTime(p.delivery_time) AS finish_date, p.sender_name, p.receiver_address, p.receiver_name,p.receiver_phone,p.zone_code,p.zone_name,p.forwarding_cost,
    p.df_payer,
    cod,price,
...
foreach($rows as $row){
 ...
 return DV::depends(1,[
  ...
  function getOverDueMerchants($no_cache =0){
    $cache_key =   'overduemerchant_he1281';
    ...
    $card1 = (object)['package_count'=>0, 'merchant_count'=>0,'title'=>'Overdue AR','subtitle'=>'Over 24h','name'=>'over_ar_24h']; /** AR over 24h*/
    $card2 = (object)['package_count'=>0, 'merchant_count'=>0,'title'=>'Overdue AD','subtitle'=>'Over 24h','name'=>'over_ap_24h']; /** AP over 24h*/

    /** IMPORTANT NOTE: The rare problem here is WHEN the the item has positive price but the price is lower than the delivery fees. This case is now NOT covered */
    /** We do not consider fee_payer = Sender | Receiver. => Here all fees are always paid by Sender or merchants */
    $sql = 'SELECT COUNT(p.id) AS package_count,SUM(p.base_fee + IFNULL(p.delivery_fee,0) + IFNULL(p.forwarding_cost,0)) AS total, s.id AS sender_id,s.name,s.code,s.phone_number 
    FROM package as p INNER JOIN sender AS s ON s.id = p.sender_id 
    WHERE p.status_id = 8 AND TIMESTAMPDIFF(HOUR, p.delivery_time, NOW()) >=24 AND IFNULL(p.sender_pmt_status_id,0) = 0 AND (IFNULL(p.cod,0) =0 OR p.price <=0)
    GROUP BY s.id,s.name,s.code,s.phone_number HAVING total >0';
    $rows = DB::select(DB::raw($sql));
    foreach($rows as $row){
      $card1->merchant_count++;
      $card1->package_count += $row->package_count;
    }

    /** We do not consider fee_payer = Sender | Receiver. => Here all fees are always paid by Sender or merchants */
    $sql = 'SELECT COUNT(p.id) AS package_count, SUM(p.price - p.base_fee - p.delivery_fee - IFNULL(p.forwarding_cost,0)) AS total, s.id AS sender_id,s.name,s.code,s.phone_number 
    FROM package as p INNER JOIN sender AS s ON s.id = p.sender_id 
    WHERE p.status_id = 8 AND TIMESTAMPDIFF(HOUR, p.delivery_time, NOW()) >=24 AND IFNULL(p.sender_pmt_status_id,0) = 0 AND p.price > 0
    GROUP BY s.id,s.name,s.code,s.phone_number HAVING total > 0';
    $rows = DB::select(DB::raw($sql));
    foreach($rows as $row){
       $card2->merchant_count++;
       $card2->package_count += $row->package_count;
    }
    ...
    * */  
    function getOutstandingPayments_merchant($arr,$sender_id = null, $ss=null){
        ...
        //if(strtolower($ss->user_class) !== 'merchant'){
           /** getOverDueDrivers() uses cached data for 2 minutes interval */
           $od_cards = $this->getOverDueMerchants($d->no_cache); 
        //}

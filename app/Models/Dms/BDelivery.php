...
class BDelivery //extends Model
{
    ...
    function b_assignDeliveryDriver($arr,$ss) {
        ...
        if ($d->is_from_mobile) $driver_id = $ss->official_id; 
        ...
        $nowTime = getNowTime();
        $inputs = ['warehouse_id'=>$m->warehouse_id,'driver_id'=>$driver_id,'delivery_id'=>$delivery_id,'status_id'=>$status_id,'last_checkout_time'=>$nowTime];
        if($continue_to_deliver == 1) $inputs['failed_num'] = 1;
        else  $inputs['first_checkout_time'] = $nowTime;
       ...
       //end::notify to concerned driver
        $delivered = $continue_to_deliver? 0 : ($package->status_id==8? 1:0);
        return DV::success(['continue_to_deliver'=>$continue_to_deliver,'delivered'=>$delivered]);
      }

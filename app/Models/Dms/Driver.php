 ...
 function getDriverInfoById($d){
  ...
  static function resetCodes($branch_id,$prefix){
    ...
    foreach($rows as $row){
        ...
        DB::table('driver')->where('id',$row->id)->update(['code'=>$new_code]);
        DB::table('um_users')->where('official_id',$row->id)->where('user_class','driver')->update(['official_code'=>$new_code]);
        $i++;
    }
  ...
  static function updateDriverPhone($id,$phone_number){
   ...
   //saveDriver() | updateDriver() | createDriver()
    ...
    function getOverDueDrivers($no_cache =0){
        $cache_key =   'overduedriver_he1290';
        $data = Cache::get($cache_key);
        ...
        $card1 = (object)['package_count'=>0, 'driver_count'=>0,'title'=>'Overdue (4h-24h)','subtitle'=>'4h-24h','name'=>'4_24h']; /** between 4h and 24h*/
        $card2 = (object)['package_count'=>0, 'driver_count'=>0,'title'=>'Overdue (Over 24h)','subtitle'=>'Over 24h','name'=>'over_24h']; /** Over 24h*/

        $sql = 'SELECT COUNT(p.id) AS package_count, p.driver_id,d.name,d.code,d.phone_number 
        FROM package as p INNER JOIN driver AS d ON d.id = p.driver_id 
        WHERE TIMESTAMPDIFF(HOUR, p.delivery_time, NOW()) >=4 AND TIMESTAMPDIFF(HOUR, p.delivery_time, NOW()) <=24 AND p.status_id = 8 AND IFNULL(p.driver_pmt_status_id,0) = 0 AND p.driver_id > 0
        GROUP BY p.driver_id,d.name,d.code,d.phone_number';
        ...
        $sql = 'SELECT COUNT(p.id) AS package_count, p.driver_id,d.name,d.code,d.phone_number 
        FROM package as p INNER JOIN driver AS d ON d.id = p.driver_id 
        WHERE TIMESTAMPDIFF(HOUR, p.delivery_time, NOW()) >24 AND IFNULL(p.driver_pmt_status_id,0) = 0 AND p.status_id = 8 AND p.driver_id > 0
        GROUP BY p.driver_id,d.name,d.code,d.phone_number';
        $rows = DB::select(DB::raw($sql));
        foreach($rows as $row){
          $card2->driver_count++;
    ...
    function getBalanceDues($arr = [], $id = null, $ss = null)
    {
        ...
        $alert_name = isset($d->alert_name)?$d->alert_name:null;
       ...
       $alert_sql = '7=7';
      /** if driver_id is provided, Do not apply alert filter*/
      if(!$id || $id ==-1){
        $alert_filters = [
          'overude-all'=>'7=7',
          '4_24h'=>'TIMESTAMPDIFF(HOUR, p.delivery_time, NOW()) >=4 AND TIMESTAMPDIFF(HOUR, p.delivery_time, NOW()) <=24',
          'over_24h'=>'TIMESTAMPDIFF(HOUR, p.delivery_time, NOW()) > 24'
        ];
        if($alert_name) $alert_sql = $alert_filters[$alert_name];
      } 
       ...
       if($ss->user_class !== 'driver'){
          ...
          $od_cards = $this->getOverDueDrivers($d->no_cache); 
       }
       $data = (object)[

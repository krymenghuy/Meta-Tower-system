  ...
  function getComboItems_country($d){
      ...
      if($ss->status_code !==200) return $ss; //user not authenticated
       //need permission to do this task
        ...
        $rows = DB::table('loc_countries AS c')->whereRaw("c.branch_id =0 OR c.branch_id =$branch_id")->selectRaw('c.id AS country_id,c.name, c.name_kh')->orderByRaw('c.name ASC')->get();  
		    return $rows;
	}
      ...
      function createCountry($d){
        ...
        if($ss->status_code !==200) return $ss; //user not authenticated
         //need permission to do this task
        $branch_id = Sanitizer::sanitize($ss->branch_id);
        if ($this->shared_loc_data === 1)  $branch_id =0;
...
return null;
  ...
  function cityExists($uss,$country_id,$name,$id) {
    ...
    if($id > 0)
     return  DB::table('loc_cities AS c')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.country_id',$country_id)->where('c.name',$name)->where('c.id','<>',$id)->limit(1)->exists(); 
    else 
     return  DB::table('loc_cities AS c')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.country_id',$country_id)->where('c.name',$name)->limit(1)->exists();     
 }
 ...
 function districtExists($uss,$city_id,$name,$id) {
    ...
    $branch_id = Sanitizer::sanitize($uss->branch_id);
    if($id > 0)
    return  DB::table('loc_districts AS c')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.city_id',$city_id)->where('c.name',$name)->where('c.id','<>',$id)->limit(1)->exists(); 
    else 
    return  DB::table('loc_districts AS c')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.city_id',$city_id)->where('c.name',$name)->limit(1)->exists();     
 }
...
function communeExists($uss,$city_id,$name,$id) {
  ...
  $branch_id = Sanitizer::sanitize($uss->branch_id);
  if($id > 0)
   return  DB::table('loc_communes AS c')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.district_id',$city_id)->where('c.name',$name)->where('c.id','<>',$id)->limit(1)->exists(); 
  else 
   return  DB::table('loc_communes AS c')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.district_id',$city_id)->where('c.name',$name)->limit(1)->exists();     
}
  ...
  function countryExists($uss,$name,$id) {
        ...
        $branch_id = Sanitizer::sanitize($uss->branch_id);
        if($id > 0)
          $rows = DB::table('loc_countries as c')->selectRaw('c.id')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.name',$name)->where('c.id','<>',$id)->limit(1)->get();
        else 
        $rows = DB::table('loc_countries as c')->selectRaw('c.id')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.name',$name)->limit(1)->get();
    ...
    function getCountryList($d){
        ...
        $rows = DB::table('loc_countries AS c')->selectRaw('c.id,c.name,c.name_kh')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->get(); 
        return $rows;
    }
      ...
      function getDistrictList($d){
       $ss = UM::getUserInfoByToken($d);
        ...
        $rows= DB::table('loc_districts AS z')->join('loc_cities AS c','c.id','=','z.city_id')->join('loc_countries AS c1','c1.id','=','c.country_id')->selectRaw('z.id,z.name,z.name_kh,c1.id as country_id,c1.name AS country_name,c.id as city_id,c.name AS city_name')->whereRaw("z.branch_id = $branch_id OR z.branch_id =0")->where('z.city_id',$city_id)->get();   
        return $rows;
    }
      ...
      function getCityList($d){
        ...
        $branch_id = Sanitizer::sanitize($ss->branch_id);
        $country_id = $d->country_id; 
        $rows = DB::table('loc_cities AS c')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.country_id',$country_id)->selectRaw('c.id,c.name')->get(); 
        return $rows; 
    }
    ...
    function getCommuneList($d){
      ...
      $district_id = isset($d->district_id)?$d->district_id:0;
        $rows = DB::table('loc_communes AS c')->join('loc_districts AS d','d.id','=','c.district_id')->join('loc_cities AS c1','c1.id','=','d.city_id')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.district_id',$district_id)->selectRaw('c.id,c.name,c.name_kh, d.name_kh AS district_name, c1.name_kh AS city_name')->get();  
        return $rows;
    }
  ...
  function deleteCommune($d){
    ...
    return null;
      ...
      function getComboItems_city($d){
        ...
        $branch_id = Sanitizer::sanitize($ss->branch_id);
        $country_id = Sanitizer::sanitize($d->country_id); 
        $rows  = DB::table('loc_cities')->whereRaw("branch_id = $branch_id OR branch_id =0")->where('country_id',$country_id)->selectRaw('id AS city_id,name,name_kh')->get();
        return $rows;
    }
    ...
    function getComboItems_district($d){
      ...
      $branch_id = Sanitizer::sanitize($ss->branch_id);
      $city_id = Sanitizer::sanitize($d->city_id); 
      $rows  = DB::table('loc_districts AS d')->whereRaw("d.branch_id = $branch_id OR d.branch_id =0")->where('d.city_id',$city_id)->selectRaw('d.id AS district_id,d.name,d.name_kh')->orderByRaw('d.name ASC')->get();
      return $rows;
  }
  ...
  function getComboItems_commune($d){
    ...
    $branch_id = Sanitizer::sanitize($ss->branch_id);
    $district_id = Sanitizer::sanitize($d->district_id); 
    $rows  = DB::table('loc_communes AS c')->whereRaw("c.branch_id = $branch_id OR c.branch_id =0")->where('c.district_id',$district_id)->selectRaw('c.id AS commune_id,c.name,c.name_kh')->orderByRaw('c.name ASC')->get();
    return $rows;
  }

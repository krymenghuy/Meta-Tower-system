...
class Lead //extends Model
{
    ...
    function save($arr=[],$id =null,$ss=null){
      ...
      if($id > 0){
           ...
           if ($lead_created){
              ...
              $created_from_mobile ='';
              if($ss->user_class =='sales_agent') $created_from_mobile = ' from Mobile App';
              $des = $ss->full_name.' created a new lead named '.$inputs['name'].' at '.date('d M Y H:i', strtotime('now')).$created_from_mobile;
              self::trackStatus($ss,$id,1,$des);
           }
  ...
  function updateStatus($status_id,$id=null,$ss=null){
    ...
    $x = DB::table('leads')->where('id',$id)->update([
        'status_id'=>$status_id,
        'closing_status_id'=>$status_id,
        'closing_date'=>$nowTime,
        'client_id'=>null,
    ...
    if($x){
      self::trackStatus($ss,$id,$status_id,null);
    }
  ...
  static function trackStatus($ss,$lead_id,$status_id,$des = null){
    $new_status = DB::table('lead_statuses as ls')->where('id',$status_id)->take(1)->value('name');
    $des = $des ?? $ss->full_name.' changed lead status to '.$new_status.' at '.date('d M Y H:i', strtotime('now'));
    $inputs = ['lead_id'=>$lead_id,'status_id'=>$status_id,'remarks'=>$des];
    $id = saveData($ss,'lead_status_tracks',['id'=>null],$inputs,[],1,false);
  ...
  function convertToMerchant($id=null,$ss =null){
     ...
     if($res->status =='OK'){
      ...
      $x = DB::table('leads')->where('id',$id)->update([
       ...
       if($x){
          //Now save new merchant successfully, so update leads's status tp Won (5), and set lead's closing_status_id to WON(5), and update lead's closing_date
          $won_status_id = 5;
          DB::table('leads')->where('id',$id)->update([
             'status_id'=>$won_status_id,
             'closing_status_id'=>$won_status_id,
             'closing_date'=>date('Y-m-d'),
             'update_uid'=>$ss->user_id,
             'update_user'=>$ss->full_name,
             'update_date'=>getNowTime()
          ]);
          $des = $ss->full_name.' converted lead '.$lead->name.' to merchant (change status to WON) at '.date('d M Y H:i', strtotime('now'));
          self::trackStatus($ss,$id,$won_status_id,$des);

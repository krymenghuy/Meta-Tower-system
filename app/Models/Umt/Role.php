...
class Role //extends Model
{
  ...
  function addPermission($prn, $role_id = null, $ss = null)
  {
    ...
    $module_id = null;
    $module_name = null;
    $row = DB::table('um_permissions as p')->join('um_app_modules as m', 'm.id', '=', 'p.module_id')
    ->where('p.id', $prn_id)
    ->selectRaw('p.module_id,p.app_id,m.name,p.category')
    ->first();
    \Log::info(json_encode(DB::table('um_permissions as p')->where('id', 213)->selectRaw('p.module_id,p.app_id')->first()));

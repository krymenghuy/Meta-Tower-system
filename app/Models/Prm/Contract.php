...
//  Set building_spaces to Available when no Active/Pending contract remains on that space.
    ...
    public function getListPaginate($arr, $ss = null)
    {
        ...
        $latestRenewalId = "(SELECT cr.id FROM contract_renewals cr WHERE cr.contract_id = c.id ORDER BY cr.id DESC LIMIT 1) AS latest_renewal_id";
        $latestRenewalStart = "(SELECT DATE(cr.start_date) FROM contract_renewals cr WHERE cr.contract_id = c.id ORDER BY cr.id DESC LIMIT 1) AS latest_renewal_start_date";
        $latestRenewalSpaceId = "(SELECT cr.space_id FROM contract_renewals cr WHERE cr.contract_id = c.id ORDER BY cr.id DESC LIMIT 1) AS latest_renewal_space_id";
        $hasPendingRenewal = "(CASE WHEN (SELECT DATE(cr.start_date) FROM contract_renewals cr WHERE cr.contract_id = c.id ORDER BY cr.id DESC LIMIT 1) > CURDATE() THEN 1 ELSE 0 END) AS has_pending_renewal";
        $canManagePendingRenewal = "(CASE WHEN (SELECT DATE(cr.start_date) FROM contract_renewals cr WHERE cr.contract_id = c.id ORDER BY cr.id DESC LIMIT 1) > CURDATE() THEN 1 ELSE 0 END) AS can_manage_pending_renewal";

        $selectCols = 'c.id,c.tenant_id,t.name as tenant_name,t.code,t.email,t.phone_number,c.legal_name,c.status_id,cs.name as status,' . $start_date . ',' . $end_date . ',' . $lastRenewalDate . ',' . $latestRenewalId . ',' . $latestRenewalStart . ',' . $latestRenewalSpaceId . ',' . $hasPendingRenewal . ',' . $canManagePendingRenewal . ',c.business_type_id,bt.name as business_type,c.space_type_id,st.name as space_type,c.space_id, bs.code as space_code,c.sqm_size,c.price,c.price_type,c.deposit,c.remarks,c.update_user,' . $updated_at . '';
        $query = DB::table('contracts as c')
    ...
    protected static function getLatestRenewalRowByContractId($contractId)
    {
        ...
        return DB::table('contract_renewals')
            ...
            ->orderByDesc('id')
            ->select('id', 'contract_id', 'space_id', 'start_date', 'end_date', 'remarks')
            ->first();
    }

    protected static function isRenewalStartInFuture($renewalStartDate, $today = null): bool
    {
        $today = $today ?? date('Y-m-d');
        if (!$renewalStartDate) return false;
        $renewStart = date('Y-m-d', strtotime((string) $renewalStartDate));
        return $renewStart > $today;
    }
    ...
    public function cancelPendingRenewal($id, $ss = null)
    {
        ...
        $latest = self::getLatestRenewalRowByContractId((int) $id);
        if (!$latest) {
            return DV::error('No renewal found for this contract.');
        }

        $today = date('Y-m-d');
        if (!self::isRenewalStartInFuture($latest->start_date ?? null, $today)) {
            return DV::error('Renewal start date has already started. You cannot cancel this renewal.');
        }

        DB::beginTransaction();
        try {
            $deleted = DB::table('contract_renewals')
    ...
    public function updatePendingRenewal($arr = [], $id = null, $ss = null)
    {
        ...
        $today = date('Y-m-d');
        if (!self::isRenewalStartInFuture($latest->start_date ?? null, $today)) {
            return DV::error('Renewal start date has already started. You cannot modify this renewal.');
        }
        ...
        $newSpaceId = !empty($inputs['space_id']) ? (int) $inputs['space_id'] : (int) ($latest->space_id ?? 0);
        if ($newSpaceId <= 0) {
            return DV::error('Please select a valid unit code.');
        }

        // Prevent choosing a unit that already has a live contract (excluding this contract).
        $dup_id = self::checkDuplicateContract($newSpaceId, (int) $id);
        if ($dup_id) {
            return DV::error('The selected unit already has a contract.');
        }

        // Prevent overlapping renewals on the selected unit.
        $overlapRenewal = self::findOverlappingRenewalOnUnit(
            $newSpaceId,
            $latest->start_date,
            $latest->end_date,
        ...
        if ($overlapRenewal) {
            ...
            $msg = 'Renewal dates overlap an existing contract period for unit '
                ...
                . '.';
            $conflictStart = $overlapRenewal->start_date
                ? date('d-M-Y', strtotime((string) $overlapRenewal->start_date))
                : '';
            $conflictEnd = $overlapRenewal->end_date
                ? date('d-M-Y', strtotime((string) $overlapRenewal->end_date))
                : $conflictStart;
            if ($conflictStart !== '' && $conflictEnd !== '') {
                $msg .= ' (' . $conflictStart . ' – ' . $conflictEnd . ')';
            }
        ...
        if (array_key_exists('remarks', $inputs)) {
            $update['remarks'] = trim((string) ($inputs['remarks'] ?? ''));
        }
        $update['updated_at'] = getNowTime();
        $update['update_uid'] = $ss->user_id ?? null;
        $update['update_user'] = $ss->full_name ?? null;

        DB::beginTransaction();
        try {
            $saved = DBX::saveData($ss, 'contract_renewals', ['id' => (int) $latest->id], $update, [], 1);
            if (!$saved) {
                DB::rollBack();

...
class Receipt extends Model
{
    ...
    public static function getReceiptDetails($id)
{
    // 1. Fetch the main receipt header details
    $header = DB::table('receipts as r')
        ->leftJoin('tenants as t', 't.id', '=', 'r.tenant_id')
        ->leftJoin('invoices as i', 'i.id', '=', 'r.invoice_id')
        ->where('r.id', $id)
    ...
    // 2. Calculate the cumulative total_paid up to this receipt's creation time
    // This sums all previous and current receipts for this specific invoice
    $header->total_paid = DB::table('receipts')
        ->where('invoice_id', $header->invoice_id)
        ->where('id', '<=', $id) // Ensures it only tracks up to the current transaction sequence
        ->sum('total_received');

    // 3. Format the calculated totals explicitly as formatted decimals/floats
    $header->total_received = (float) $header->total_received;
    $header->total_paid = (float) $header->total_paid;

    // 4. Fetch the payment methods breakdown used for this specific receipt
    $header->breakdowns = DB::table('receipt_breakdowns as rb')
        ->leftJoin('banks as b', 'b.id', '=', 'rb.bank_id')
        ->where('rb.receipt_id', $id)
        ->select(

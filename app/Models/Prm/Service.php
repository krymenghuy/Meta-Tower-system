  ...
  public function saveService($arr = [], $id = null, $ss = null)
    ...
    public function getListPaginate($arr, $ss = null){
        ...
        $query = DB::table('services as s')
            ...
            ->selectRaw("s.id,s.name,s.category_id,sc.name as service_category,s.type_id,s.level,(CASE WHEN s.level=1 THEN 'Standard' ELSE 'Premium' END) as service_level, st.name as service_type,s.charge_as,s.price,s.status_id,ss.name as status,s.description,s.updated_at,s.update_user")
            ->orderBy('s.id','DESC');
    ...
    public static function serviceDetails($id,$ss = null){
        return DB::table('services as s')
            ->where('s.id',$id)
            ->selectRaw('s.id,s.name,s.category_id,s.type_id,s.level,s.charge_as,s.price,s.status_id,s.description')
            ->first();
    }

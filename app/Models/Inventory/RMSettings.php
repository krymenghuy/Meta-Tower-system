...
class RMSettings extends Model
{
    use HasFactory;
    ...
    static function options_warehouse($ss){
        $branch_id = $ss->branch_id;
        return DB::table('warehouses as c')->where('c.item_class',self::$item_class)->where('c.branch_id',$branch_id)->select("c.id","c.name as warehouse_name")->orderByRaw('c.name ASC')->get();
    }
    ...
    static function options_item($ss){
        $branch_id = $ss->branch_id;
        return DB::table('inv_items AS i')->where('i.branch_id',$branch_id)->where('i.item_class',self::$item_class)->selectRaw("i.id as `value`,i.name as text")->orderByRaw('i.name ASC')->get();
    }

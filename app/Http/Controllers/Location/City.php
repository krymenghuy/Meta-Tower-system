...
class City //extends Model
{
    ...
    static function list($country_id=null,$ss){
        ...
        return DB::table('loc_cities AS c')->whereRaw($str_country)->join('loc_countries as p','p.id','=','c.country_id')->select('c.id','c.name','c.name_kh','c.country_id','p.name as country')->orderBy('c.name','ASC')->get();
    }
    static function save($d,$ss){
        $sanitize_rules = [];
        $branch_id = $ss->branch_id;
        $check_unique = ["$branch_id|loc_cities|name|id=id"];
        $res = DBX::validateObject($d,['id'=>'0|number|identity=1','name'=>'1|string|1-100','name_kh'=>'0|string|0-100','country_id'=>'1|number'],true,$sanitize_rules,$ss->lang,false,$check_unique);
        if($res->error) return DV::error($res->error);
        $id = $res->id;
        $inputs = $res->values;
        $name_kh = $inputs['name_kh'];
        $name_kh = $name_kh?$name_kh:$inputs['name'];
        $id = DBX::saveData($ss,'loc_cities',['id'=>$id],$inputs,[],1,0);
    ...
    function getCityFromOption($id = null,$ss = null){
        $id = $id?$id:$this->id;
        $city_detail = DB::table('loc_cities')->where('id',$id)->selectRaw('id,name,name_kh')->first();

        return (object)[
            'zone'=>$city_detail ?? null
        ];
    }

...
class SignupController extends Controller
{
    function apiSignup(Request $request)
    {
        ...
        $signup_name = $request->signup_name;
        $pwd = $request->password;

        $result = XAuthService::verifyUser($app_id, $signup_name, $pwd);
        // if ($result->status === 'OK') {
    ...
    function processSignup(Request $request){
        ...
        $result = XAuthService::verifyUser($THIS_APP_ID,$request->signup_name,$request->password,"en");
        if ($result->status_code ===200) {
            $user = $result->user;
                $access_token =Crypt::encryptString($user->access_token);
                unset($user->access_token);
                 XAuthService::signup($user);
            ...
            Session::put('signup_error',$result->error_message);
            return redirect('');
        }

...
class BenefitController extends Controller
{
    ...
    public function getBenefitList(Request $req)
    {
        $ss = AuthService::verifyAuth($req, -1);
        if ($ss->status_code !== 200) {
            return JDV::raw($ss);
        }
        return JDV::result($this->benefitModel->getBenefitList( $ss));
    }

    public function getBenefitListPaginate(Request $req)
    {
        $ss = AuthService::verifyAuth($req, -1);
        if ($ss->status_code !== 200) {
            return JDV::raw($ss);
        }
        return JDV::result($this->benefitModel->getBenefitListPaginate($req->all(), $ss));
    }
    ...
    public function getDetails(Request $req)
    {
        ...
        return JDV::result($this->benefitModel->getDetails($req->id, $ss));
    }

...
class RMItemGroupController extends Controller
{
    ...
    function saveItemGroup(Request $req){
        ...
        if($ss->status_code !=200) return $ss; //user not authenticated
        $d = $req->all(); 
        $res = RMItemGroup::commitSave($ss,$d);
    ...
    function deleteItemGroup(Request $req){
        ...
        if($ss->status_code !=200) return $ss; //user not authenticated
        $id = $req->id;
        $res = RMItemGroup::commitDelete($ss,$id);
        return JDV::raw($res);
    }
    ...
    function getGroupInfo(Request $req){
        ...
        if($ss->status_code !=200) return $ss; //user not authenticated
        $id = $req->id;
        return JDV::result(RMItemGroup::info($ss,$id));
    }

...
class CurrencyController extends Controller
{
    ...
    function getExchangeRateInfo(Request $req){
        ...
        $id = $req->id;
        $col_date = DBX::formatDate('r.create_date','create_date');
        $cols = 'r.id,r.x_date,r.x_month,r.currency_pair,ROUND(r.buy_rate,2) AS buy_rate,ROUND(r.sell_rate,2) AS sell_rate,r.create_user,'.$col_date;
        $row= DB::table('exchange_rates as r')->where('id',$id)->where('subs_id',$bin_subs_id)->selectRaw($cols)->first();
    ...
    function getExchangeRates(Request $req){
        ...
        $query = DB::table("exchange_rates as r")->whereRaw($str_period)->whereRaw($str_pair)->where('r.subs_id',$bin_subs_id)->selectRaw('r.id,formatDate(r.x_date) as x_date,r.currency_pair,ROUND(r.buy_rate,2) AS buy_rate,ROUND(r.sell_rate,2) AS sell_rate')->orderByRaw("r.x_date DESC");
